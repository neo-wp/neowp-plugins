...
\NeoDuplicate\NeoGlobal\register_backend_page("neo-log", fn () => \NeoDuplicate\NeoGlobal\current_user_can__neo_log__view(), function () {
    $log_dir = \NeoDuplicate\NeoGlobal\cache_path("neo-log");
    $warnings = [];
    $file_infos = [];
    $all_logs = [];
    $per_file_limit = (int) (\NeoDuplicate\NeoGlobal\query_param("limit") ?? 100);

    if (\NeoDuplicate\NeoGlobal\query_param("download-all-logs") === "1") {
        ...
        $zip_dir_path = \NeoDuplicate\NeoGlobal\cache_path("neo-log__download");
        $zip_file_path = $zip_dir_path . "/neo-log--all-plugins-" . \NeoDuplicate\NeoGlobal\unique_planck_date() . ".zip";
        $download_filename = (wp_parse_url(site_url(), PHP_URL_HOST) ?: "unknown") . "_" . \NeoDuplicate\NeoGlobal\wp_date_string("Y-m-d H-i-s") . "_neo-log.zip";
        $log_dir_paths = array_merge(\NeoDuplicate\NeoGlobal\fs_glob($cache_dir_path . "/*/neo-log", GLOB_ONLYDIR) ?: [], \NeoDuplicate\NeoGlobal\fs_glob($cache_dir_path . "/*/neo-log--*", GLOB_ONLYDIR) ?: []);
        $log_dir_paths = \NeoDuplicate\NeoGlobal\array_unique_better($log_dir_paths); sort($log_dir_paths, SORT_STRING);
        try {
            $zip = new \ZipArchive();
            if ($zip->open($zip_file_path, \ZipArchive::CREATE | \ZipArchive::OVERWRITE) !== true) { \NeoDuplicate\NeoGlobal\throw_global_exception("Could not create neoLog ZIP."); }
            $added_files_count = 0;
            foreach ($log_dir_paths as $log_dir_path) {
                ...
                foreach (\NeoDuplicate\NeoGlobal\iterate_all_files($log_dir_path) as $log_file_path) {
                    if (!\NeoDuplicate\NeoGlobal\fs_is_file($log_file_path)) { continue; }
            ...
            header("Content-Length: " . \NeoDuplicate\NeoGlobal\fs_filesize($zip_file_path));
            \NeoDuplicate\NeoGlobal\fs_readfile($zip_file_path);
        } finally {
            if (\NeoDuplicate\NeoGlobal\fs_file_exists($zip_file_path)) { \NeoDuplicate\NeoGlobal\fs_unlink($zip_file_path); }
        }
        exit;
    }

    if (!\NeoDuplicate\NeoGlobal\fs_is_dir($log_dir)) { echo "Warning: The log directory does not exist: " . esc_html($log_dir); exit; }
    $log_files = \NeoDuplicate\NeoGlobal\iterate_all_files($log_dir);

    foreach ($log_files as $full_path) {
        if (basename($full_path) === "_all.log") { continue; }

        $file_size_bytes = \NeoDuplicate\NeoGlobal\fs_filesize($full_path);
        ...
        for ($i = $last_line_num; $i >= 0 && $line_counter < $per_file_limit; $i--) {
            ...
            $decoded = \NeoDuplicate\NeoGlobal\json_decode_better($line, suppress_error: true); if ($decoded === false) { $warnings[] = "Warning: Could not decode JSON line in file $full_path: $line"; continue; }
            $decoded["_raw"] = $line;
            if (!isset($decoded["date"])) { $warnings[] = "Warning: Missing 'date' in file {$full_path}: {$line}"; continue; }
            if (!isset($decoded["message"])) { $json_string = \NeoDuplicate\NeoGlobal\json_encode_better($decoded); if (mb_strlen($json_string) > 1000) { $json_string = mb_substr($json_string, 0, 1000) . "..."; } $decoded["message"] = $json_string; }
            $file_logs[] = ["date" => $decoded["date"], "message" => $decoded["message"], "raw" => $decoded["_raw"]]; $line_counter++;
        }
        unset($file_obj);

        $tokenize_message = function ($message) {
            $tokens = \NeoDuplicate\NeoGlobal\preg_split_better('/(\s+|[{}\[\],:])/', $message, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
        ...
        $common_prefix = function ($list_of_token_arrays) {
            ...
            for ($i = 0; $i < $smallest_number_of_elements; $i++) {
                $all_the_same = count(\NeoDuplicate\NeoGlobal\array_unique_better(array_column($list_of_token_arrays, $i))) === 1;
        ...
        foreach ($deduplicated as $group) {
            ...
            $duration = \NeoDuplicate\NeoGlobal\timestamp_from_utc_date_string($date_newest) - \NeoDuplicate\NeoGlobal\timestamp_from_utc_date_string($date_oldest);
            ...
            $middle_texts = \NeoDuplicate\NeoGlobal\array_unique_better($middle_texts);
            $middle_texts_non_empty = \NeoDuplicate\NeoGlobal\array_filter_better($middle_texts, function ($middle_text) { return $middle_text !== ""; });
...
<head>
    ...
    <?php \NeoDuplicate\NeoGlobal\backend_page_script_tag_start([]); ?> /* Own script tag instead of enqueue because it's our own HTML page */
        <?php echo \NeoDuplicate\NeoGlobal\get_code_for_js_variables("backend") /* phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped */ ?>;
    <?php \NeoDuplicate\NeoGlobal\backend_page_script_tag_end(); ?>
    <?php \NeoDuplicate\NeoGlobal\backend_page_style_tag_start([]); ?> /* Own style tag instead of enqueue because it's our own HTML page */
...
<p>Currently reading max <?php echo esc_html($per_file_limit) ?> lines per file (<a href="<?php echo esc_url(\NeoDuplicate\NeoGlobal\add_query_param(\NeoDuplicate\NeoGlobal\request_uri(), "limit", 1000)); ?>">change to 1000</a>).</p>
...
<table id="logFileTable">
    ...
    <?php foreach ($file_infos as $fi) {
        ...
        ?>
        <tr>
            <td class="<?php echo esc_attr($file_class) ?>"><a href="<?php echo esc_url(\NeoDuplicate\NeoGlobal\cache_url("neo-log") . "/" . $file_name) ?>" target="_blank"><?php echo esc_html($file_name) ?></a></td>
...
<button onclick="window.location.href = '<?php echo esc_url(\NeoDuplicate\NeoGlobal\add_query_param(\NeoDuplicate\NeoGlobal\request_uri(), "download-all-logs", "1")); ?>'">Download All Logs</button>
...
<?php \NeoDuplicate\NeoGlobal\backend_page_script_tag_start(["type" => "module"]); ?>
    import { fetchEndpoint } from '<?php echo esc_url(\NeoDuplicate\NeoGlobal\plugin_url() . "/_global--endpoint.js"); ?>';
    import { extractJson } from '<?php echo esc_url(\NeoDuplicate\NeoGlobal\plugin_url() . "/_global--extract-json.js"); ?>';
...
\NeoDuplicate\NeoGlobal\register_rest_endpoint("/wp-json/neo/delete-logs", "POST", fn () => \NeoDuplicate\NeoGlobal\current_user_can__neo_log__view(), function () {
    \NeoDuplicate\NeoGlobal\delete_all(\NeoDuplicate\NeoGlobal\cache_path("neo-log"));
    return "OKAY";
});
