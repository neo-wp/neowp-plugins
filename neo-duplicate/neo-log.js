...
const sendNeoLogToEndpoint = async (type, message) => {
    try {
        if (!fetchEndpoint.nonce && !window["neoGlobalEnqueueBackendNeoDuplicate"]?.neoGlobalRestEndpointNonce && !window["neoGlobalEnqueueFrontendNeoDuplicate"]?.neoGlobalRestEndpointNonce) { return; }
        const now = Date.now();
