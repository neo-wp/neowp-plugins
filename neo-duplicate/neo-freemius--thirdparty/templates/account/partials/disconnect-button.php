...
<script type="text/javascript">
    ...
    ( function( $ ) {
        $( document ).ready( function() {
            ...
            $( '#<?php /* phpcs:ignoreFile */ echo $button_id ?>' ).on( 'click', function( e ) {
            ...
            $modal.on( 'click', '.button-primary', function ( evt ) {
                $( '#<?php /* phpcs:ignoreFile */ echo $button_id ?>' ).closest( 'form' )[0].submit();
            } );
        } );
    } )( jQuery );
</script>
<div id="fs_modal_delete_site" class="fs-modal active" style="display: none">
    <div class="fs-modal-dialog">
        <div class="fs-modal-header">
            <h4><?php /* phpcs:ignoreFile */ echo $fs->esc_html_inline( 'Disconnect', 'disconnect' ) ?></h4>
        </div>
        <div class="fs-modal-body">
            <?php /* phpcs:ignoreFile */ if ( ! is_object( $license ) ) : ?>
            <p><?php /* phpcs:ignoreFile */ echo
            ...
            <?php /* phpcs:ignoreFile */ if ( $has_active_subscription ) : ?>
                <p><?php /* phpcs:ignoreFile */ echo
            ...
            <p><?php /* phpcs:ignoreFile */ echo esc_html( $fs->get_text_inline( 'Are you sure you would like to proceed with the disconnection?', 'disconnect-confirm' ) ) ?></p>
        </div>
        <div class="fs-modal-footer">
            <button class="button button-primary<?php /* phpcs:ignoreFile */ if ( is_object( $license ) ) : ?> warn<?php /* phpcs:ignoreFile */ endif ?>" tabindex="2"><?php /* phpcs:ignoreFile */ echo $fs->esc_html_inline( 'Yes', 'yes' ) . ' - ' .  $fs->esc_html_inline( 'Disconnect', 'disconnect' ) ?></button>
            <button class="button button-secondary button-close" tabindex="1"><?php /* phpcs:ignoreFile */ echo esc_html( $fs->get_text_inline( 'Cancel', 'cancel' ) ) ?></button>
        </div>
    </div>
</div>
<form action="<?php /* phpcs:ignoreFile */ echo esc_attr( $fs->_get_admin_page_url( 'account' ) ); ?>" method="POST">
    <input type="hidden" name="fs_action" value="delete_account">
    <?php /* phpcs:ignoreFile */ wp_nonce_field( 'delete_account' ) ?>

    <a id="<?php /* phpcs:ignoreFile */ echo $button_id ?>" href="#" class="fs-button-inline">
        <i class="dashicons dashicons-no"></i>
        <?php /* phpcs:ignoreFile */ echo $fs->esc_html_inline( 'Disconnect', 'disconnect' ) ?>
    </a>
</form>