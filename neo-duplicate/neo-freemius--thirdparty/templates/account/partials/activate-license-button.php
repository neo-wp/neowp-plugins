...
<form action="<?php /* phpcs:ignoreFile */ echo $fs->_get_admin_page_url( 'account' ) ?>" method="POST">
    ...
    <input type="hidden" name="license_id" value="<?php /* phpcs:ignoreFile */ echo $available_license->id ?>">
    <input type="submit" class="fs-activate-license button<?php /* phpcs:ignoreFile */ echo ! empty( $VARS['class'] ) ? ' ' . $VARS['class'] : '' ?>"
           value="<?php /* phpcs:ignoreFile */ echo esc_attr( sprintf(
