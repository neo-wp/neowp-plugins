...
?>
<tr<?php /* phpcs:ignoreFile */ if ( $odd ) {
    echo ' class="alternate"';
} ?>>
    <td>
        <!-- Title -->
        <?php /* phpcs:ignoreFile */ echo $addon_info['title'] ?>
    </td>
    <?php /* phpcs:ignoreFile */ if ( $is_addon_connected ) : ?>
        <!-- ID -->
        <td><?php /* phpcs:ignoreFile */ echo $site->id ?></td>
        <!--/ ID -->

        <!-- Version -->
        <td><?php /* phpcs:ignoreFile */ echo $version ?></td>
        <!--/ Version -->

        <!-- Plan Title -->
        <td><?php /* phpcs:ignoreFile */ echo strtoupper( is_string( $plan_name ) ? $plan_title : $free_text ) ?></td>
        <!--/ Plan Title -->

        <!-- Expiration -->
        <td>
        <?php /* phpcs:ignoreFile */ if ( $site->is_trial() || is_object( $license ) ) : ?>
            <?php /* phpcs:ignoreFile */
                $tags = array();
...
//            }
        ...
        <!-- Actions -->
        <td><?php /* phpcs:ignoreFile */ if ( $buttons_count > 1 ) : ?>
            <div class="button-group"><?php /* phpcs:ignoreFile */ endif ?>
                <?php /* phpcs:ignoreFile */ foreach ( $buttons as $button ) {
                        echo $button;
                    } ?>
                <?php /* phpcs:ignoreFile */ if ( $buttons_count > 1 ) : ?></div><?php /* phpcs:ignoreFile */ endif ?></td>
        <!--/ Actions -->

    <?php /* phpcs:ignoreFile */ else : ?>
        <?php /* phpcs:ignoreFile */ // Add-on NOT Installed or was never connected.
            $is_addon_installed_by_filesystem = $fs->is_addon_installed( $addon_id );
        ?>
        <!-- Action -->
        <td colspan="<?php /* phpcs:ignoreFile */ echo ( $is_addon_installed_by_filesystem || $show_delete_install_button ) ? '5' : '4' ?>">
            <?php /* phpcs:ignoreFile */ if ( $is_addon_installed_by_filesystem ) : ?>
                <?php /* phpcs:ignoreFile */ $addon_file = $fs->get_addon_basename( $addon_id ) ?>
                <?php /* phpcs:ignoreFile */ if ( ! isset( $active_plugins_directories_map[ dirname( $addon_file ) ] ) ) : ?>
                <a class="button button-primary"
                   href="<?php /* phpcs:ignoreFile */ echo wp_nonce_url( 'plugins.php?action=activate&amp;plugin=' . $addon_file, 'activate-plugin_' . $addon_file ) ?>"
                   title="<?php /* phpcs:ignoreFile */ fs_esc_attr_echo_inline( 'Activate this add-on', 'activate-this-addon', $slug ) ?>"
                   class="edit"><?php /* phpcs:ignoreFile */ echo esc_html( $activate_text ) ?></a>
                <?php /* phpcs:ignoreFile */ endif ?>
            <?php /* phpcs:ignoreFile */ else : ?>
                <?php /* phpcs:ignoreFile */ if ( $fs->is_allowed_to_install() ) : ?>
                    <a class="button button-primary"
                       href="<?php /* phpcs:ignoreFile */ echo wp_nonce_url( self_admin_url( 'update.php?' . ( ( isset( $addon_info['has_paid_plan'] ) && $addon_info['has_paid_plan'] ) ? 'fs_allow_updater_and_dialog=true&' : '' ) . 'action=install-plugin&plugin=' . $addon_info['slug'] ), 'install-plugin_' . $addon_info['slug'] ) ?>"><?php /* phpcs:ignoreFile */ fs_esc_html_echo_inline( 'Install Now', 'install-now', $slug ) ?></a>
                <?php /* phpcs:ignoreFile */ else : ?>
                    <a target="_blank" rel="noopener" class="button button-primary"
                       href="<?php /* phpcs:ignoreFile */ echo $fs->_get_latest_download_local_url( $addon_id ) ?>"><?php /* phpcs:ignoreFile */ echo esc_html( $download_latest_text ) ?></a>
                <?php /* phpcs:ignoreFile */ endif ?>
            <?php /* phpcs:ignoreFile */ endif ?>
        </td>
        <!--/ Action -->
    <?php /* phpcs:ignoreFile */ endif ?>
    <?php /* phpcs:ignoreFile */ if ( $show_delete_install_button ) : ?>
