...
<div class="fs-checkout-process-redirect">
    <div class="fs-checkout-process-redirect__loader">
	    <?php /* phpcs:ignoreFile */ fs_include_template( 'ajax-loader.php' ); ?>
    </div>

    <div class="fs-checkout-process-redirect__content">
        <p>
		    <?php /* phpcs:ignoreFile */ echo esc_html( fs_text_inline( 'Processing, please wait and do not close or refresh this window...' ) ); ?>
        </p>
    </div>
</div>
...
<script type="text/javascript">
    jQuery(function ($) {
        var $loader = $( '.fs-checkout-process-redirect .fs-ajax-loader' ),
            action = <?php /* phpcs:ignoreFile */ echo wp_json_encode( $action ); ?>,
            data = <?php /* phpcs:ignoreFile */ echo wp_json_encode( $data ); ?>;
        ...
        var processPurchaseEvent = (<?php /* phpcs:ignoreFile */ echo $fs->apply_filters('checkout/purchaseCompleted', 'function (data) {
        ...
        function processInstall( data ) {
            ...
            $.form( '<?php /* phpcs:ignoreFile */ echo $fs_checkout->get_install_url( $fs, $plugin_id ); ?>', requestData ).submit();
        }
        ...
        function processPendingActivation( data ) {
            ...
            $.form( '<?php /* phpcs:ignoreFile */ echo $fs_checkout->get_pending_activation_url( $fs, $plugin_id ); ?>', requestData ).submit();
        }

        function syncLicense(data) {
            var redirectUrl = new URL( <?php /* phpcs:ignoreFile */ echo wp_json_encode( $fs->_get_sync_license_url( $plugin_id ) ); ?> );
        ...
        function goToAccount() {
            window.location.href = <?php /* phpcs:ignoreFile */ echo wp_json_encode( $fs->get_account_url() ) ?>;
        }
    });
</script>
