...
(function ($) {
	var reasonsHtml                    = <?php /* phpcs:ignoreFile */ echo json_encode( $reasons_list_items_html ) ?>,
	    modalHtml                      =
		    '<div class="fs-modal fs-modal-deactivation-feedback<?php /* phpcs:ignoreFile */ echo empty( $confirmation_message ) ? ' no-confirmation-message' : ''; ?>">'
		    + '	<div class="fs-modal-dialog">'
		    + '		<div class="fs-modal-header">'
		    + '		    <h4><?php /* phpcs:ignoreFile */ fs_esc_attr_echo_inline( 'Quick Feedback', 'quick-feedback' , $slug ) ?></h4>'
		    + '		</div>'
		    + '		<div class="fs-modal-body">'
		    + '			<div class="fs-modal-panel" data-panel-id="confirm"><p><?php /* phpcs:ignoreFile */ echo $confirmation_message; ?></p></div>'
		    + '			<div class="fs-modal-panel active" data-panel-id="reasons"><h3><strong><?php /* phpcs:ignoreFile */ echo esc_js( sprintf( fs_text_inline( 'If you have a moment, please let us know why you are %s', 'deactivation-share-reason' , $slug ), ( $fs->is_plugin() ? fs_text_inline( 'deactivating', 'deactivating', $slug ) : fs_text_inline( 'switching', 'switching', $slug ) ) ) ) ?>:</strong></h3><ul id="reasons-list">' + reasonsHtml + '</ul></div>'
		    + '		</div>'
		    + '		<div class="fs-modal-footer">'
			+ '         <?php /* phpcs:ignoreFile */ echo $anonymous_feedback_checkbox_html ?>'
			+ '         <label style="display: none" class="feedback-from-snooze-label"><input type="checkbox" class="feedback-from-snooze-checkbox"> <span><?php /* phpcs:ignoreFile */ fs_esc_js_echo_inline( 'Snooze this panel during troubleshooting', 'snooze-panel-during-troubleshooting', $slug ) ?></span><span style="display: none"><?php /* phpcs:ignoreFile */ fs_esc_js_echo_inline( 'Snooze this panel for', 'snooze-panel-for', $slug ) ?> <?php /* phpcs:ignoreFile */ echo $snooze_select_html ?></span></label>'
		    + '			<a href="#" class="button button-secondary button-deactivate"></a>'
		    + '			<a href="#" class="button button-secondary button-close"><?php /* phpcs:ignoreFile */ fs_esc_js_echo_inline( 'Cancel', 'cancel', $slug ) ?></a>'
		    + '		</div>'
		    + '	</div>'
		    + '</div>',
	    $modal                         = $(modalHtml),
		...
		isAnonymous                    = <?php /* phpcs:ignoreFile */ echo ( $is_anonymous ? 'true' : 'false' ); ?>,
		otherReasonID                  = <?php /* phpcs:ignoreFile */ echo Freemius::REASON_OTHER; ?>,
		dontShareDataReasonID          = <?php /* phpcs:ignoreFile */ echo Freemius::REASON_DONT_LIKE_TO_SHARE_MY_INFORMATION; ?>,
        deleteThemeUpdateData          = <?php /* phpcs:ignoreFile */ echo $fs->is_theme() && $fs->is_premium() && ! $fs->has_any_active_valid_license() ? 'true' : 'false' ?>,
        $subscriptionCancellationModal = $( '.fs-modal-subscription-cancellation-<?php /* phpcs:ignoreFile */ echo $fs->get_id() ?>' ),
        showDeactivationFeedbackForm   = <?php /* phpcs:ignoreFile */ echo ( $show_deactivation_feedback_form ? 'true' : 'false' ) ?>,
        $body                          = $( 'body' );
        ...
        $subscriptionCancellationModal.on( '<?php /* phpcs:ignoreFile */ echo $fs->get_action_tag( 'subscription_cancellation_action' ) ?>', function( evt, cancelSubscription ) {
            ...
            if ( false === cancelSubscription ) {
                ...
                $.ajax({
                    ...
                    method    : 'POST',
                    data      : {
                        action   : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_action( 'cancel_subscription_or_trial' ) ?>',
                        security : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_security( 'cancel_subscription_or_trial' ) ?>',
                    ...
                    beforeSend: function() {
                        ...
                        $subscriptionCancellationModal.find( '.fs-modal-footer .button-primary' ).text( '<?php /* phpcs:ignoreFile */ echo esc_js(
                    ...
                    success: function( result ) {
                        if ( result.success ) {
                            ...
                            $subscriptionCancellationModal.find( '.fs-modal-footer .button-primary' ).html( <?php /* phpcs:ignoreFile */ echo json_encode( sprintf(
	...
	function registerEventHandlers() {
		$body.on( 'click', '#the-list .deactivate > a', function ( evt ) {
		    if ( 0 === $( this ).next( '[data-module-id=<?php /* phpcs:ignoreFile */ echo $fs->get_id() ?>].fs-module-id' ).length ) {
		        return true;
            }
		 ...
		 */
		?>
		$('body').on('click', '.theme-browser .theme:not([data-slug=<?php /* phpcs:ignoreFile */ echo $fs->get_premium_slug() ?>]) .theme-actions .button.activate', function (evt) {
			evt.preventDefault();
		...
		$modal.on('click', '.fs-modal-footer .button', function (evt) {
			...
			if (_this.hasClass('allow-deactivate')) {
                ...
                if ( ! isReasonSelected ) {
				    if ( ! deleteThemeUpdateData ) {
                        ...
                        $.ajax({
                            ...
                            method    : 'POST',
                            data      : {
                                action   : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_action( 'delete_theme_update_data' ) ?>',
                                security : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_security( 'delete_theme_update_data' ) ?>',
                                module_id: '<?php /* phpcs:ignoreFile */ echo $fs->get_id() ?>'
                            },
                            beforeSend: function() {
                                _parent.find( '.fs-modal-footer .button-deactivate' ).text( '<?php /* phpcs:ignoreFile */ echo esc_js( fs_text_inline( 'Processing', 'processing', $slug ) ) ?>...' );
                            },
                            complete  : function() {
                                window.location.href = redirectLink;
                            }
                ...
                if ( shouldSnooze && <?php /* phpcs:ignoreFile */ echo Freemius::REASON_TEMPORARY_DEACTIVATION ?> == selectedReasonID ) {
				...
				$.ajax({
					...
					method    : 'POST',
					data      : {
						action       : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_action( 'submit_uninstall_reason' ) ?>',
						security     : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_security( 'submit_uninstall_reason' ) ?>',
						module_id    : '<?php /* phpcs:ignoreFile */ echo $fs->get_id() ?>',
						reason_id    : $radio.val(),
		...
		$modal.on('click', 'input[type="radio"]', function () {
			...
			$modal.find('.button-deactivate').html('<?php /* phpcs:ignoreFile */ echo esc_js( $submit_deactivate_text ) ?>');
			...
			if ( ! _parent.hasClass('has-input') ) {
				...
				if (isOtherReasonSelected()) {
					showMessage('<?php /* phpcs:ignoreFile */ echo esc_js( fs_text_inline( 'Kindly tell us the reason so we can improve.', 'ask-for-reason-message' , $slug ) ); ?>');
                    changeDeactivateButtonText();
                }
			}

            $anonymousFeedback.toggle( <?php /* phpcs:ignoreFile */ echo Freemius::REASON_TEMPORARY_DEACTIVATION ?> != selectedReasonID );
            $feedbackSnooze.toggle( <?php /* phpcs:ignoreFile */ echo Freemius::REASON_TEMPORARY_DEACTIVATION ?> == selectedReasonID );

            if ( <?php /* phpcs:ignoreFile */ echo Freemius::REASON_TEMPORARY_DEACTIVATION ?> == selectedReasonID ) {
		...
		var updateDeactivationButtonOnTrouble = function () {
            if ( snooze ) {
                $modal.find('.button-deactivate').html('<?php /* phpcs:ignoreFile */ echo esc_js( sprintf(
                ...
                $modal.find('.button-deactivate').html('<?php /* phpcs:ignoreFile */ echo esc_js(
                    $fs->is_plugin() ?
                        $deactivate_text :
                        sprintf( $activate_x_text, $theme_text )
                ) ?>');
	...
	function resetModal() {
        ...
        if ( isAnonymous ) {
			$anonymousFeedback.find( 'input' ).prop( 'checked', <?php /* phpcs:ignoreFile */ echo $fs->apply_filters( 'default_to_anonymous_feedback', false ) ? 'true' : 'false' ?> );
	...
	function changeDeactivateButtonText() {
	    ...
	    if (0 === $userReason.val().trim().length) {
            ...
            $deactivateButton.html('<?php /* phpcs:ignoreFile */ echo
                $fs->is_plugin() ?
                    $deactivate_text :
                    sprintf( $activate_x_text, $theme_text )
            ?>');
        } else {
            $deactivateButton.html('<?php /* phpcs:ignoreFile */ echo esc_js( $submit_deactivate_text ) ?>');
        }
    }
	...
	function updateButtonLabels() {
        ...
        if ( 'confirm' === getCurrentPanel() ) {
            $deactivateButton.text( <?php /* phpcs:ignoreFile */ echo json_encode( sprintf(
            ...
            $deactivateButton.html( <?php /* phpcs:ignoreFile */ echo json_encode( sprintf(
