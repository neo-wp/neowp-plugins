...
<script type="text/javascript">
    (function ($) {
        $(document).ready(function () {
            ...
            <?php /* phpcs:ignoreFile */ if (0 < count( $tabs )) : ?>
            if (null == $tab) {
            ...
            $tab.addClass('fs-tab');
            $tab.addClass('<?php /* phpcs:ignoreFile */ echo $fs->get_unique_affix() ?>');

            var $tabClone = null;

            <?php /* phpcs:ignoreFile */ $freemius_context_page = null ?>

            <?php /* phpcs:ignoreFile */ foreach ($tabs as $tab) : ?>
            <?php /* phpcs:ignoreFile */ $is_support_tab = ( 'wp-support-forum' == $tab['slug'] ) ?>
            // Add the Freemius tabs.
            $tabClone = $tab.clone();
            $tabClone.html(<?php /* phpcs:ignoreFile */ echo json_encode( $tab['label'] ) ?>)
                .attr('href', '<?php /* phpcs:ignoreFile */ echo $is_support_tab ? $fs->get_support_forum_url() : $tab['href'] ?>')
                .appendTo($tabsWrapper)
                // Remove any custom click events.
                .off('click', '**')
                .addClass('<?php /* phpcs:ignoreFile */ echo $tab['slug'] ?>')
                // Avoid tab click triggering parent events.
                .click(function (e) {
                    e.stopPropagation();
                });

            <?php /* phpcs:ignoreFile */ if ($is_support_tab) : ?>
            // Open support in a new tab/page.
            $tabClone.attr('target', '_blank');
            <?php /* phpcs:ignoreFile */ endif ?>

            <?php /* phpcs:ignoreFile */ if ($fs->is_admin_page( $tab['slug'] )) : ?>
            <?php /* phpcs:ignoreFile */ $freemius_context_page = $tab['slug'] ?>
            // Select the relevant Freemius tab.
            $tabs.removeClass('nav-tab-active');
            $tabClone.addClass('nav-tab-active');

            <?php /* phpcs:ignoreFile */ if (in_array( $freemius_context_page, array( 'pricing', 'contact', 'checkout' ) )) : ?>
            // Add AJAX loader.
            ...
            $tabs.each(function (j, tab) {
                if (0 === $(tab).attr('href').indexOf('#')) {
                    $(tab).attr('href', '<?php /* phpcs:ignoreFile */ echo esc_js( $fs->main_menu_url() ) ?>' + $(tab).attr('href'));
                }
            });
            <?php /* phpcs:ignoreFile */ endif ?>
...
//						window.scrollTo(0,0);
            ...
            <?php /* phpcs:ignoreFile */ if (is_string( $freemius_context_page ) && in_array( $freemius_context_page, array(
            'pricing',
            'contact',
            'checkout'
        ) )) : ?>
