...
class GlobalException extends \Exception {
    ...
    public function __construct($message, $status_code = 500, $error_code = "neo_global__endpoint_error") {
        if (\function_exists(__NAMESPACE__ . "\\is_interface_system_available") && \NeoDuplicate\NeoGlobal\is_interface_system_available()) { [$additional_message_for_errors, $interface_ok] = \NeoDuplicate\NeoGlobal\call_interface_func_implemented('\NeoDuplicate\NeoLog\interface_additional_message_for_errors_20250505')(); } else { $interface_ok = false; }
        ...
        if (\function_exists(__NAMESPACE__ . "\\global_log")) { \NeoDuplicate\NeoGlobal\global_log_with_module_name("_global--exceptions", "$message in " . $this->getTraceAsString()); \NeoDuplicate\NeoGlobal\global_log_with_module_name("_global--exceptions", "Error: $message in " . $this->getTraceAsString(), log_source: "error"); }
    }
...
class CurlException extends \NeoDuplicate\NeoGlobal\GlobalException {
    private $url;
...
function throw_global_exception($message, $status_code = 500, $error_code = "neo_global__endpoint_error") {
    throw new \NeoDuplicate\NeoGlobal\GlobalException($message, $status_code, $error_code); /* phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped */ /* Our custom GlobalException class above already escapes all inputs properly. */
}

function throw_curl_exception($url, $http_status_code, $response_body) {
    throw new \NeoDuplicate\NeoGlobal\CurlException($url, $http_status_code, $response_body); /* phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped */ /* Our custom CurlException class above already escapes all inputs properly through GlobalException. */
}
