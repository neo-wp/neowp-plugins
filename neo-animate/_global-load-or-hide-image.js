export async function loadOrHideImage(imgNode, imageUrl) {
    ...
    try {
        ...
        if (responseBlob.size < 10000) { imgNode.closest("neo-setting-neo-animate").style.display = "none"; return; }
        blobUrl = URL.createObjectURL(responseBlob);
    } catch (error) {
        imgNode.closest("neo-setting-neo-animate").style.display = "none"; return;
    }
