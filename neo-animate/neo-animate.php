<?php
/*
* Plugin Name: neo Animate
* Plugin URI: https://neo-wp.com/?plugin-slug=neo-animate&edition=wporg&ref=entry-file
* Description: Make an impression with animated line-path effects!
* Version: 26.8.31
* Requires PHP: 8.0
...
* Edition: wporg
* Slug: neo-animate
* Filename: neo-animate.26.8.31.zip
* Date: 2026-08-31 17:34:04 (Europe/Berlin)
...
namespace NeoAnimate\NeoGlobal {
...
namespace NeoAnimate\NeoEntrypoint {
    // Abort if this plugin is loaded twice with different names.
    if (!function_exists("\\" . __NAMESPACE__ . "\\" . "dummy_function")) { function dummy_function() { }
        ...
        function suppress_plugin_on_error($callback) {
            try { return $callback(); }
                ...
                if (\NeoAnimate\NeoGlobal\str_contains($error_message, "#ignoreNeoSuppress")) { throw $e; }
                if (defined("WP_CLI") && WP_CLI) { throw $e; }
                try {
                    if (!($e instanceof \NeoAnimate\NeoGlobal\GlobalException)) {
                        if (function_exists('\NeoAnimate\NeoGlobal\global_warn')) { \NeoAnimate\NeoGlobal\global_warn_with_module_name("neowp", "PHP Error in plugin: $error_message"); }
                    }
                } catch (\Throwable $inner_e) {}
                suppress_plugin($error_message);

                if (file_exists(__DIR__ . "/_global--error-message-page-only-debug.php")) { (fn () => require_once(__DIR__ . "/_global--error-message-page-only-debug.php"))(); \NeoAnimate\NeoGlobal\show_better_error_message_page($e); }
        ...
        suppress_plugin_on_error(function () {
            ...
            if (\NeoAnimate\NeoGlobal\str_contains($query_string, "neosuppress")) {
                suppress_plugin("Plugin is suppressed by ?neosuppress in the URL.");
            }
            if (\NeoAnimate\NeoGlobal\str_contains($query_string, "neoundosuppress")) {
            ...
            if (!empty($neo_suppress_cookie)) {
                ...
                if (\NeoAnimate\NeoGlobal\str_contains($query_string, "page=" . \NeoAnimate\NeoGlobal\plugin_settings_page_slug())) { add_action("init", function () { wp_safe_redirect(admin_url()); exit; }); }
            ...
            if (in_array(PHP_OS_FAMILY, \NeoAnimate\NeoGlobal\plugin_blocked_os_families()))                { $neo_load_error_message = "neoPlugin is inactive because it is not supported on " . PHP_OS_FAMILY . "."; }
            if (!version_compare(PHP_VERSION, \NeoAnimate\NeoGlobal\plugin_min_php_ver(), ">="))            { $neo_load_error_message = "neoPlugin is inactive because PHP is too old (" . PHP_VERSION . " < " . \NeoAnimate\NeoGlobal\plugin_min_php_ver() . ")."; }
            if (!version_compare(get_bloginfo("version"), \NeoAnimate\NeoGlobal\plugin_min_wp_ver(), ">=")) { $neo_load_error_message = "neoPlugin is inactive because WP is too old (" . get_bloginfo("version") . " < " . \NeoAnimate\NeoGlobal\plugin_min_wp_ver() . ")."; }
            ...
            function _neo_entry_function_get_module_name($rel_path) {
                if (\NeoAnimate\NeoGlobal\str_starts_with($rel_path, "_global-")) { return "neo-global"; }
            ...
            foreach (scandir(dirname(__FILE__)) as $plugin_file_name) {
                ...
                if (!\NeoAnimate\NeoGlobal\str_ends_with($plugin_file_name, ".php"))                { continue; }
                if ($plugin_file_name === "index.php")                                   { continue; }
                if ($plugin_file_name === basename(\NeoAnimate\NeoGlobal\plugin_entry_file_path())) { continue; }
                if (\NeoAnimate\NeoGlobal\str_starts_with($plugin_file_name, "_global-")) { $global_files[] = $plugin_file_name; }
            ...
            \NeoCore\NeoSuperGlobal::$globals["active_plugins"][] = [
                "slug" => \NeoAnimate\NeoGlobal\plugin_slug(), "edition" => \NeoAnimate\NeoGlobal\plugin_edition(),
                "version" => \NeoAnimate\NeoGlobal\plugin_version(),
                "namespace_prefix" => \NeoAnimate\NeoGlobal\plugin_namespace_prefix(),
                "plugin_path" => dirname(__FILE__),
                "plugin_entry_file_path" => \NeoAnimate\NeoGlobal\plugin_entry_file_path(),
            ...
            function master_function($is_special_call_for_activation_or_uninstallation = false) {
                suppress_plugin_on_error(function () use ($is_special_call_for_activation_or_uninstallation) {
                    ...
                    \NeoAnimate\NeoGlobal\performance_checkpoint("live update", time: $live_update_time_before, is_start: true);
                    \NeoAnimate\NeoGlobal\performance_checkpoint("live update", time: $live_update_time_after,  is_end: true);
                    \NeoAnimate\NeoGlobal\performance_checkpoint("plugin-load", is_start: true);
                    ...
                    foreach (\NeoCore\NeoSuperGlobal::$globals["active_plugins"] as $plugin) {
                        foreach ($plugin["global_files"] as $global_file) {
                            ...
                            \NeoAnimate\NeoGlobal\performance_checkpoint("load $global_file", is_start: true);
                            (fn () => require_once($plugin["plugin_path"] . "/" . $global_file))();
                            \NeoAnimate\NeoGlobal\performance_checkpoint("load $global_file", is_end: true);
                        }
                    }

                    if (\NeoAnimate\NeoGlobal\array_filter_better(\NeoCore\NeoSuperGlobal::$globals["active_plugins"], fn ($active_plugin) => array_key_exists("neo-redirect", $active_plugin["module_files"]))) {
                    ...
                    if (\NeoAnimate\NeoGlobal\array_filter_better(\NeoCore\NeoSuperGlobal::$globals["active_plugins"], fn ($active_plugin) => array_key_exists("neo-manager-installer", $active_plugin["module_files"]))) {
                        foreach (\NeoCore\NeoSuperGlobal::$globals["active_plugins"] as &$active_plugin_ref) {
                            foreach ($active_plugin_ref["module_files"] as $module_slug => $module_files) {
                                foreach (\NeoAnimate\NeoGlobal\option__neo_manager_installer__suppressed_plugins_list() as $suppressed_plugin_slug) {
                                    if (\NeoAnimate\NeoGlobal\str_starts_with($module_slug, $suppressed_plugin_slug)) {
                    ...
                    foreach (\NeoCore\NeoSuperGlobal::$globals["active_plugins"] as $active_plugin) {
                        foreach ($active_plugin["module_files"] as $module_slug => $module_files) {
                            ...
                            foreach ($module_files as $module_file) {
                                ...
                                \NeoAnimate\NeoGlobal\performance_checkpoint("load $module_file", is_start: true);
                                (fn () => require_once($active_plugin["plugin_path"] . "/" . $module_file))();
                                \NeoAnimate\NeoGlobal\performance_checkpoint("load $module_file", is_end: true);
                            }
                        }
                    }
                    ...
                    \NeoAnimate\NeoGlobal\performance_checkpoint("plugin-load", is_end: true);
                });
            }

            \NeoCore\NeoCoreMaster::register_master_function(\NeoAnimate\NeoGlobal\plugin_timestamp(), __NAMESPACE__ . "\\master_function");
