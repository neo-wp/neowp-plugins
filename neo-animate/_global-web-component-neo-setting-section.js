...
export default class NeoSettingSection extends HTMLElement {
    ...
    constructor() {
        ...
        this._isOpen = getCookie("neo-web-component-setting-section-open-neo-animate--" + this.getAttribute("id")) === "true";
    ...
    connectedCallback() {
        const BODY_CSS_ID = "neo-setting-section-neo-animate";
        if (!document.getElementById(BODY_CSS_ID)) {
            ...
            globalCss.textContent =`
                :where(neo-setting-section-neo-animate [slot="settings"] > *) { width: 800px; max-width: 100%;}
                :where(neo-setting-section-neo-animate [slot="settings"]) { display: flex; flex-direction: column; gap: var(--neo-setting-section-space-y) var(--neo-setting-section-space-x); } /* #suppressLinterModuleNameDividedCheck */
                :where(neo-setting-section-neo-animate [slot="settings"] > *) { padding-top: var(--neo-setting-section-space-y); border-top: 1px solid rgba(0, 0, 0, 0.08); } /* #suppressLinterModuleNameDividedCheck */
            `;
    ...
    _syncOpenState({ suppressAnimation = false } = {}) {
        ...
        if (this._isOpen) { setCookie("neo-web-component-setting-section-open-neo-animate--" + this.getAttribute("id"), "true", (5) * 60); } else { removeCookie("neo-web-component-setting-section-open-neo-animate--" + this.getAttribute("id")); }
