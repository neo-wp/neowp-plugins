...
class NeoURL {
    ...
    public function to_string() {
        ...
        $path_parts = $this->path; if (!empty($path_parts) && count(\NeoAnimate\NeoGlobal\array_filter_better($path_parts, fn ($p) => $p !== "")) === 0) { $path_parts[]= ""; }
    ...
    public function remove_params($keys) {
        $this->params = \NeoAnimate\NeoGlobal\array_filter_better($this->params, function ($param) use ($keys) { return !(in_array($param[0], $keys) || in_array(urldecode($param[0]), $keys)); });
        return $this;
    }
...
function make_internal_url_relative_to_uploads($img_url) {
    ...
    static $relative_uploads_url = null; $relative_uploads_url ??= (new NeoURL(\NeoAnimate\NeoGlobal\uploads_url()))->make_relative()->to_string();
                                         $relative_img_url       = (new NeoURL($img_url))                ->make_relative()->to_string();
    return \NeoAnimate\NeoGlobal\str_replace_start(
...
function percent_encode_invalid_utf8_url_bytes(string $url): string {
    ...
    return \NeoAnimate\NeoGlobal\preg_replace_callback_better("/[\\x80-\\xFF]/", fn ($byte) => "%" . strtoupper(bin2hex($byte[0])), $url);
}
...
function percent_encode_invalid_utf8_bytes_recursive($value) {
    if (is_string($value)) { return mb_check_encoding($value, "UTF-8") ? $value : \NeoAnimate\NeoGlobal\preg_replace_callback_better("/[\\x80-\\xFF]/", fn ($byte) => "%" . strtoupper(bin2hex($byte[0])), $value); }
...
function get_path_from_internal_url($url) {
    if (!is_url_internal($url)) { \NeoAnimate\NeoGlobal\throw_global_exception("No internal URL: " . $url); }
