...
<div class="fs-modal fs-modal-auto-install">
	<div class="fs-modal-dialog">
		<div class="fs-modal-header">
			<h4><?php /* phpcs:ignoreFile */ echo esc_js( fs_text_inline( 'Automatic Installation', 'auto-installation', $slug ) ) ?></h4>
		</div>
		<div class="fs-modal-body">
			<div class="fs-notice-error" style="display: none"><p></p></div>
			<?php /* phpcs:ignoreFile */ if ( $require_credentials ) : ?>
				<div id="request-filesystem-credentials-dialog">
					<?php /* phpcs:ignoreFile */ echo $credentials_form ?>
				</div>
			<?php /* phpcs:ignoreFile */ else : ?>
				<p class="fs-installation-notice"><?php /* phpcs:ignoreFile */ echo sprintf(
			...
			<p class="fs-installing"
			   style="display: none"><?php /* phpcs:ignoreFile */ echo sprintf( fs_esc_html_inline( 'The installation process has started and may take a few minutes to complete. Please wait until it is done - do not refresh this page.', 'installing-module-x', $slug ), $plugin_title ) ?></p>
		</div>
		<div class="fs-modal-footer">
			<?php /* phpcs:ignoreFile */ echo $loader_html ?>
			<button
				class="button button-secondary button-cancel"><?php /* phpcs:ignoreFile */ fs_esc_html_echo_inline( 'Cancel Installation', 'cancel-installation', $slug ) ?><?php /* phpcs:ignoreFile */ if ( ! $require_credentials ) : ?> (<?php /* phpcs:ignoreFile */ echo $countdown_html ?>)<?php /* phpcs:ignoreFile */ endif ?></button>
			<button
				class="button button-primary"><?php /* phpcs:ignoreFile */ fs_esc_html_echo_inline( 'Install Now', 'install-now', $slug ) ?></button>
		</div>
	</div>
</div>'
	...
	(function ($) {
		$(document).ready(function () {
			var $modal             = $('.fs-modal-auto-install'),
			    ...
			    requireCredentials = <?php /* phpcs:ignoreFile */ echo json_encode( $require_credentials ) ?>,
			...
			var startAutoInstall = function () {
				...
				var data = {
					action          : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_action( 'install_premium_version' ) ?>',
					security        : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_security( 'install_premium_version' ) ?>',
					slug            : '<?php /* phpcs:ignoreFile */ echo $slug ?>',
					module_id       : '<?php /* phpcs:ignoreFile */ echo $fs->get_id() ?>',
                    target_module_id: '<?php /* phpcs:ignoreFile */ echo $plugin_id ?>'
				};
				...
				$.ajax({
					...
					success: function (resultObj) {
						...
						if (resultObj.success) {
							...
							switch (resultObj.error.code) {
								...
								default:
									...
									$modal.find('.button-cancel').html(<?php /* phpcs:ignoreFile */ fs_json_encode_echo_inline( 'Cancel Installation', 'cancel-installation', $slug ) ?>);
						...
						if (reloadAccount) {
							window.location = '<?php /* phpcs:ignoreFile */ echo $fs->get_account_url() ?>';
						}
			...
			var countdown         = <?php /* phpcs:ignoreFile */ echo $sec_countdown ?>,
