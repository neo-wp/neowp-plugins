    ...
    class FS_Plugin_Info_Dialog {
        ...
        function _get_addon_info_filter( $data, $action = '', $args = null ) {
            ...
            if ( ! $has_paid_plan && $selected_addon->is_wp_org_compliant ) {
                $repo_data = FS_Plugin_Neo_Updater::_fetch_plugin_info_from_repository(
                ...
                $latest = $this->_fs->_fetch_latest_version(
                    $selected_addon->id,
                    true,
                    FS_Plugin_Neo_Updater::UPDATES_CHECK_CACHE_EXPIRATION,
                ...
                <div class="button-group">
                    ...
                    <div class="button button-primary fs-dropdown-arrow-button">
                        ...
                        <ul class="fs-dropdown-list" style="display: none">
                            <?php /* phpcs:ignoreFile */ for ( $i = 1; $i < $total_actions; $i ++ ) : ?>
                                <li><?php /* phpcs:ignoreFile */ echo str_replace( 'button button-primary', '', $actions[ $i ] ) ?></li>
...
//	var_dump($api->banners);
            if ( ! empty( $api->banners ) && ( ! empty( $api->banners['low'] ) || ! empty( $api->banners['high'] ) ) ) {
                ...
                <style type="text/css">
                    ...
                    {
                        background-image: url( <?php /* phpcs:ignoreFile */ echo esc_url( $low ); ?> );
                    }
                    ...
                    @media only screen and ( -webkit-min-device-pixel-ratio: 1.5 )
                    {
                        #plugin-information-title.with-banner
                        {
                            background-image: url( <?php /* phpcs:ignoreFile */ echo esc_url( $high ); ?> );
                        }
                    }
                </style>
                <?php /* phpcs:ignoreFile */
            ...
            ?>
        <div id="<?php /* phpcs:ignoreFile */ echo $_tab; ?>-content" class='<?php /* phpcs:ignoreFile */ echo $_with_banner; ?>'>
            <div class="fyi">
                <?php /* phpcs:ignoreFile */ if ( $api->is_paid ) : ?>
                    <?php /* phpcs:ignoreFile */ if ( isset( $api->plans ) ) : ?>
                        <div class="plugin-information-pricing">
                        <?php /* phpcs:ignoreFile */ foreach ( $api->plans as $plan ) : ?>
                            <?php /* phpcs:ignoreFile */
                             ...
                             */
                            ?>
                            <?php /* phpcs:ignoreFile */ $first_pricing = $plan->pricing[0] ?>
                            <?php /* phpcs:ignoreFile */ $is_multi_cycle = $first_pricing->is_multi_cycle() ?>
                            <div class="fs-plan<?php /* phpcs:ignoreFile */ if ( ! $is_multi_cycle ) {
                                echo ' fs-single-cycle';
                            } ?>" data-plan-id="<?php /* phpcs:ignoreFile */ echo $plan->id ?>">
                                <h3 data-plan="<?php /* phpcs:ignoreFile */ echo $plan->id ?>"><?php /* phpcs:ignoreFile */ echo esc_html( sprintf( fs_text_x_inline( '%s Plan', 'e.g. Professional Plan', 'x-plan', $api->slug ), $plan->title ) ) ?></h3>
                                <?php /* phpcs:ignoreFile */ $has_annual = $first_pricing->has_annual() ?>
                                <?php /* phpcs:ignoreFile */ $has_monthly = $first_pricing->has_monthly() ?>
                                <div class="nav-tab-wrapper">
                                    <?php /* phpcs:ignoreFile */ $billing_cycles = array( 'monthly', 'annual', 'lifetime' ) ?>
                                    <?php /* phpcs:ignoreFile */ $i = 0;
                                        foreach ( $billing_cycles as $cycle ) : ?>
                                            <?php /* phpcs:ignoreFile */ $prop = "{$cycle}_price";
                                            if ( isset( $first_pricing->{$prop} ) ) : ?>
                                                <?php /* phpcs:ignoreFile */ $is_featured = ( 'annual' === $cycle && $is_multi_cycle ) ?>
                                                <?php /* phpcs:ignoreFile */
                                                $prices = array();
                                                foreach ( $plan->pricing as $pricing ) {
                                                ...
                                                ?>
                                                <a class="nav-tab" data-billing-cycle="<?php /* phpcs:ignoreFile */ echo $cycle ?>"
                                                   data-pricing="<?php /* phpcs:ignoreFile */ echo esc_attr( json_encode( $prices ) ) ?>">
                                                    <?php /* phpcs:ignoreFile */ if ( $is_featured ) : ?>
                                                        <label>
                                                            &#9733; <?php /* phpcs:ignoreFile */ fs_esc_html_echo_x_inline( 'Best', 'e.g. the best product', 'best', $api->slug ) ?>
                                                            &#9733;</label>
                                    ...
                                    <?php /* phpcs:ignoreFile */ wp_enqueue_script( 'jquery' ) ?>
                                        ...
                                        (function ($, undef) {
                                            var
                                                _formatBillingFrequency = function (cycle) {
                                                    switch (cycle) {
                                                        case 'monthly':
                                                            return '<?php /* phpcs:ignoreFile */ printf( fs_text_x_inline( 'Billed %s', 'e.g. billed monthly', 'billed-x', $api->slug ), fs_text_x_inline( 'Monthly', 'as every month', 'monthly', $api->slug ) ) ?>';
                                                        case 'annual':
                                                            return '<?php /* phpcs:ignoreFile */ printf( fs_text_x_inline( 'Billed %s', 'e.g. billed monthly', 'billed-x', $api->slug ), fs_text_x_inline( 'Annually', 'as once a year', 'annually', $api->slug ) ) ?>';
                                                        case 'lifetime':
                                                            return '<?php /* phpcs:ignoreFile */ printf( fs_text_x_inline( 'Billed %s', 'e.g. billed monthly', 'billed-x', $api->slug ), fs_text_x_inline( 'Once', 'as once a year', 'once', $api->slug ) ) ?>';
                                                    }
                                                },
                                                _formatLicensesTitle    = function (pricing) {
                                                    switch (pricing.licenses) {
                                                        case 1:
                                                            return '<?php /* phpcs:ignoreFile */ fs_esc_attr_echo_inline( 'Single Site License', 'license-single-site', $api->slug ) ?>';
                                                        case null:
                                                            return '<?php /* phpcs:ignoreFile */ fs_esc_attr_echo_inline( 'Unlimited Licenses', 'license-unlimited', $api->slug ) ?>';
                                                        default:
                                                            return '<?php /* phpcs:ignoreFile */ fs_esc_attr_echo_inline( 'Up to %s Sites', 'license-x-sites', $api->slug ) ?>'.replace('%s', pricing.licenses);
                                                    }
                                                },
                                                _formatPrice            = function (pricing, cycle, multipleLicenses) {
                                                    ...
                                                    switch (cycle) {
                                                        case 'monthly':
                                                            priceCycle = ' / <?php /* phpcs:ignoreFile */ fs_echo_x_inline( 'mo', 'as monthly period', 'mo', $api->slug ) ?>';
                                                            break;
                                                        case 'lifetime':
                                                            priceCycle = '';
                                                            break;
                                                        case 'annual':
                                                        default:
                                                            priceCycle = ' / <?php /* phpcs:ignoreFile */ fs_echo_x_inline( 'year', 'as annual period', 'year', $api->slug ) ?>';
                                                            break;
                                                    }
                                                ...
                                                _checkoutUrl            = function (plan, pricing, cycle) {
                                                    return '<?php /* phpcs:ignoreFile */ echo esc_url_raw( remove_query_arg( 'billing_cycle', add_query_arg( array( 'plugin_id' => $plan->plugin_id ), $api->checkout_link ) ) ) ?>' +
                                                        '&plan_id=' + plan +
                                                        '&pricing_id=' + pricing +
                                                        '&billing_cycle=' + cycle<?php /* phpcs:ignoreFile */ if ( $plan->has_trial() ) {
                                                        echo " + '&trial=true'";
                                                    }?>;
                                                },
                                            ...
                                            $(document).ready(function () {
                                                var $plan = $('.plugin-information-pricing .fs-plan[data-plan-id=<?php /* phpcs:ignoreFile */ echo $plan->id ?>]');
                                                ...
                                                $plan.find('.nav-tab').click(function () {
                                                    ...
                                                    if (1 == pricing.length) {
                                                        html = '<li><label><?php /* phpcs:ignoreFile */ echo fs_esc_attr_x_inline( 'Price', 'noun', 'price', $api->slug ) ?>: ' + _formatPrice(pricing[0], billingCycle, false) + '</label></li>';
                                                    } else {
                                                        for (var i = 0; i < pricing.length; i++) {
                                                            html += '<li><label><input name="pricing-<?php /* phpcs:ignoreFile */ echo $plan->id ?>" type="radio" value="' + pricing[i].id + '">' + _formatPrice(pricing[i], billingCycle) + '</label></li>';
                                                        }
                                                    }
                                                ...
                                                <?php /* phpcs:ignoreFile */ if ( $has_annual ) : ?>
                                ...
                                <div class="fs-pricing-body">
                                    ...
                                    <?php /* phpcs:ignoreFile */ $annual_discount = ( $has_annual && $has_monthly ) ? $plan->pricing[0]->annual_discount_percentage() : 0 ?>
                                    <?php /* phpcs:ignoreFile */ if ( $annual_discount > 0 ) : ?>
                                        <span
                                            class="fs-annual-discount"><?php /* phpcs:ignoreFile */ printf(
                                    ...
                                    <?php /* phpcs:ignoreFile */ echo $this->get_actions_dropdown( $api, $plan ) ?>
                                    <div style="clear:both"></div>
                                    <?php /* phpcs:ignoreFile */ if ( $plan->has_trial() ) : ?>
                                        <?php /* phpcs:ignoreFile */ $trial_period = $this->get_trial_period( $plan ) ?>
                                        <ul class="fs-trial-terms">
                                            <li>
                                                <i class="dashicons dashicons-yes"></i><?php /* phpcs:ignoreFile */ echo esc_html( sprintf( fs_text_inline( 'No commitment for %s - cancel anytime', 'no-commitment-x', $api->slug ), $trial_period ) ) ?>
                                            </li>
                                            <li>
                                                <i class="dashicons dashicons-yes"></i><?php /* phpcs:ignoreFile */ printf( esc_html( fs_text_inline( 'After your free %s, pay as little as %s', 'after-x-pay-as-little-y', $api->slug ) ), $trial_period, '<var class="fs-price">' . $this->get_price_tag( $plan, $plan->pricing[0] ) . '</var>' ) ?>
                                            </li>
                                        </ul>
                                    <?php /* phpcs:ignoreFile */ endif ?>
                                </div>
                ...
                <?php /* phpcs:ignoreFile */ endif ?>
                <div>
                    <h3><?php /* phpcs:ignoreFile */ fs_echo_inline( 'Details', 'details', $api->slug ) ?></h3>
                    <ul>
                        <?php /* phpcs:ignoreFile */ if ( ! empty( $api->version ) ) { ?>
                            <li>
                                <strong><?php /* phpcs:ignoreFile */ fs_esc_html_echo_x_inline( 'Version', 'product version', 'version', $api->slug ); ?>
                                    :</strong> <?php /* phpcs:ignoreFile */ echo $api->version; ?></li>
                            <?php /* phpcs:ignoreFile */
                        }
                            if ( ! empty( $api->author ) ) {
                                ?>
                                <li>
                                    <strong><?php /* phpcs:ignoreFile */ fs_echo_x_inline( 'Author', 'as the plugin author', 'author', $api->slug ); ?>
                                        :</strong> <?php /* phpcs:ignoreFile */ echo links_add_target( $api->author, '_blank' ); ?>
                                </li>
                                <?php /* phpcs:ignoreFile */
                            }
                            if ( ! empty( $api->last_updated ) ) {
                                ?>
                                <li><strong><?php /* phpcs:ignoreFile */ fs_echo_inline( 'Last Updated', 'last-updated', $api->slug ); ?>
                                        :</strong> <span
                                        title="<?php /* phpcs:ignoreFile */ echo $api->last_updated; ?>">
                ...
                /* translators: %s: time period (e.g. "2 hours" ago) */
                    ...
                    human_time_diff( strtotime( $api->last_updated ) )
                            ...
                            if ( ! empty( $api->requires ) ) {
                                ?>
                                <li>
                                    <strong><?php /* phpcs:ignoreFile */ fs_esc_html_echo_inline( 'Requires WordPress Version', 'requires-wordpress-version', $api->slug ) ?>
                                        :</strong> <?php /* phpcs:ignoreFile */ echo esc_html( sprintf(
                            ...
                            if ( ! empty( $api->tested ) ) {
                                ?>
                                <li>
                                    <strong><?php /* phpcs:ignoreFile */ fs_esc_html_echo_inline( 'Compatible up to', 'compatible-up-to', $api->slug ); ?>
                                        :</strong> <?php /* phpcs:ignoreFile */ echo $api->tested; ?>
                                </li>
                                <?php /* phpcs:ignoreFile */
                            }
                            if ( ! empty( $api->requires_php ) ) {
                                ?>
                                <li>
                                    <strong><?php /* phpcs:ignoreFile */ fs_esc_html_echo_inline( 'Requires PHP Version', 'requires-php-version', $api->slug ); ?>:</strong>
                                    <?php /* phpcs:ignoreFile */
                            ...
                            if ( ! empty( $api->downloaded ) ) {
                                ?>
                                <li>
                                    <strong><?php /* phpcs:ignoreFile */ fs_esc_html_echo_inline( 'Downloaded', 'downloaded', $api->slug ) ?>
                                        :</strong> <?php /* phpcs:ignoreFile */ echo esc_html( sprintf(
                                        ( ( 1 == $api->downloaded ) ?
                            ...
                            if ( ! empty( $api->slug ) && true == $api->is_wp_org_compliant ) {
                                ...
                                <li><a target="_blank"
                                       ...
                                       href="https://wordpress.org/plugins/<?php /* phpcs:ignoreFile */ echo $api->slug; ?>/"><?php /* phpcs:ignoreFile */ fs_esc_html_echo_inline( 'WordPress.org Plugin Page', 'wp-org-plugin-page', $api->slug ) ?>
                                        &#187;</a>
                                </li>
                                <?php /* phpcs:ignoreFile */
                            }
                            if ( ! empty( $api->homepage ) ) {
                                ...
                                <li><a target="_blank"
                                       rel="noopener noreferrer"
                                       href="<?php /* phpcs:ignoreFile */ echo esc_url( $api->homepage ); ?>"><?php /* phpcs:ignoreFile */ fs_esc_html_echo_inline( 'Plugin Homepage', 'plugin-homepage', $api->slug ) ?>
                                        &#187;</a>
                                </li>
                                <?php /* phpcs:ignoreFile */
                            }
                            if ( ! empty( $api->donate_link ) && empty( $api->contributors ) ) {
                                ...
                                <li><a target="_blank"
                                       rel="noopener noreferrer"
                                       href="<?php /* phpcs:ignoreFile */ echo esc_url( $api->donate_link ); ?>"><?php /* phpcs:ignoreFile */ fs_esc_html_echo_inline( 'Donate to this plugin', 'donate-to-plugin', $api->slug ) ?>
                                        &#187;</a>
                                </li>
                            <?php /* phpcs:ignoreFile */ } ?>
                    </ul>
                </div>
                <?php /* phpcs:ignoreFile */ if ( ! empty( $api->rating ) ) { ?>
                    <h3><?php /* phpcs:ignoreFile */ fs_echo_inline( 'Average Rating', 'average-rating', $api->slug ); ?></h3>
                    <?php /* phpcs:ignoreFile */ wp_star_rating( array(
                        'rating' => $api->rating,
                        'type'   => 'percent',
                        'number' => $api->num_ratings
                    ) ); ?>
                    <small>(<?php /* phpcs:ignoreFile */ echo esc_html( sprintf(
                        ...
                        foreach ( $api->ratings as $key => $ratecount ) {
                            ...
                            <div class="counter-container">
                              <span class="counter-label"><a
                                href="https://wordpress.org/support/view/plugin-reviews/<?php /* phpcs:ignoreFile */ echo $api->slug; ?>?filter=<?php /* phpcs:ignoreFile */ echo $key; ?>"
                                target="_blank"
                                rel="noopener noreferrer"
                                title="<?php /* phpcs:ignoreFile */ echo esc_attr( sprintf(
                                  /* translators: %s: # of stars (e.g. 5 stars) */
                                  fs_text_inline( 'Click to see reviews that provided a rating of %s', 'click-to-reviews', $api->slug ),
                                  $stars_label
                                ) ) ?>"><?php /* phpcs:ignoreFile */ echo $stars_label ?></a></span>
                                <span class="counter-back">
                                <span class="counter-bar" style="width: <?php /* phpcs:ignoreFile */ echo absint(92 * $_rating); ?>px;"></span>
                              </span>
                              <span class="counter-count"><?php /* phpcs:ignoreFile */ echo number_format_i18n( $ratecount ); ?></span>
                            </div>
                            <?php /* phpcs:ignoreFile */
                        }
                    }
                    if ( ! empty( $api->contributors ) ) {
                        ?>
                        <h3><?php /* phpcs:ignoreFile */ fs_echo_inline( 'Contributors', 'contributors', $api->slug ); ?></h3>
                        ...
                        <?php /* phpcs:ignoreFile */ if ( ! empty( $api->donate_link ) ) { ?>
                            <a target="_blank"
                               rel="noopener noreferrer"
                               href="<?php /* phpcs:ignoreFile */ echo esc_url( $api->donate_link ); ?>"><?php /* phpcs:ignoreFile */ fs_echo_inline( 'Donate to this plugin', 'donate-to-plugin', $api->slug ) ?>
                                &#187;</a>
                        <?php /* phpcs:ignoreFile */ } ?>
                    <?php /* phpcs:ignoreFile */ } ?>
            </div>
