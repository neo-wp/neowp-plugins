...
\NeoAnimate\NeoGlobal\val("neo_global__option_defaults", []); \NeoAnimate\NeoGlobal\val("neo_global__option_autoload", []); function _register_option($option_name, $default_value, bool $autoload) { if ($default_value === null) { \NeoAnimate\NeoGlobal\throw_global_exception("Option default value cannot be null"); } \NeoAnimate\NeoGlobal\val("neo_global__option_defaults")[$option_name] = $default_value; \NeoAnimate\NeoGlobal\val("neo_global__option_autoload")[$option_name] = $autoload; }
function get_or_set_option($name, $value = null, $delete = false) {
    static $cache = [];
    if ($delete && $value !== null) { \NeoAnimate\NeoGlobal\throw_global_exception("Cannot set and delete an option at the same time"); }
    if ($delete) { delete_option($name); unset($cache[$name]); return; }
    $autoload = \NeoAnimate\NeoGlobal\val("neo_global__option_autoload")[$name] ?? true;
    if ($value === null) {
        if (isset($cache[$name])) { return $cache[$name]; }
        $option_value = get_option($name);
        $default = \NeoAnimate\NeoGlobal\val("neo_global__option_defaults")[$name] ?? null;
        if ($default === null) { \NeoAnimate\NeoGlobal\throw_global_exception("Option '$name' not registered with a default value in _global--options.php"); }
        ...
        try {
            $value = \NeoAnimate\NeoGlobal\json_decode_better($option_value);
            $cache[$name] = $value; return $value;
        } catch (\NeoAnimate\NeoGlobal\GlobalException $e) {
            \NeoAnimate\NeoGlobal\global_warn_with_module_name("_global--options", "Error decoding option '$name'. Setting default value instead. " . $e->getMessage());
            get_or_set_option($name, $default);
            $cache[$name] = $default; return $default;
        }
    } else {
        $cache[$name] = $value;
        if (is_string(\NeoAnimate\NeoGlobal\val("neo_global__option_defaults")[$name] ?? null)) {
            if (!is_string($value)) { \NeoAnimate\NeoGlobal\throw_global_exception("Option '$name' must be a string because the default value is a string"); }
            $option_updated = update_option($name, $value, $autoload);
        } else { $option_updated = update_option($name, \NeoAnimate\NeoGlobal\json_encode_better($value), $autoload); }
        if (!$option_updated) { return; }
        $encoded_value = \NeoAnimate\NeoGlobal\json_encode_better($value); $encoded_value_for_log = in_array($name, ["neo_ai__connection", "neo_ai__free_provider_secret", "neo_tool_sync__pw", "neo_support__support_codes", "neo_support_codes__codes"], true) ? "\"***\"" : $encoded_value; \NeoAnimate\NeoGlobal\global_log_with_module_name("_global--options", "Setting option '$name' to value " . (mb_strlen($encoded_value_for_log) > 100 ? mb_substr($encoded_value_for_log, 0, 100) . "…" : $encoded_value_for_log));
    }
}
...
_register_option("neo_global__last_migration_plugin_timestamp_neo_animate", 0,                        true);       function option__neo_global__last_migration_plugin_timestamp_neo_animate($value = null, $delete = false) { return get_or_set_option_by_function_name(__FUNCTION__, $value, $delete); }
...
\NeoAnimate\NeoGlobal\add_action_hook("neo_init", function () {
    \NeoAnimate\NeoGlobal\register_rest_endpoint("/wp-json/neo/option-get-neo-animate", "GET", fn () => \NeoAnimate\NeoGlobal\current_user_can__global_options(), function ($get_param) {
        $option_name = $get_param("name");
        if (!array_key_exists($option_name, \NeoAnimate\NeoGlobal\val("neo_global__option_defaults"))) { \NeoAnimate\NeoGlobal\throw_global_exception("Option '$option_name' is not registered in _global--options.php"); }
        return get_or_set_option($option_name);
    });
});

\NeoAnimate\NeoGlobal\add_action_hook("neo_init", function () {
    \NeoAnimate\NeoGlobal\register_rest_endpoint("/wp-json/neo/option-set-neo-animate", "POST", fn () => \NeoAnimate\NeoGlobal\current_user_can__global_options(), function ($get_param) {
        $option_name = $get_param("name");
        if (!array_key_exists($option_name, \NeoAnimate\NeoGlobal\val("neo_global__option_defaults"))) { \NeoAnimate\NeoGlobal\throw_global_exception("Option '$option_name' is not registered in _global--options.php"); }
