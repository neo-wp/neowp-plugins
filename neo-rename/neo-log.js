...
const sendNeoLogToEndpoint = async (type, message) => {
    try {
        if (!fetchEndpoint.nonce && !window["neoGlobalEnqueueBackendNeoRename"]?.neoGlobalRestEndpointNonce && !window["neoGlobalEnqueueFrontendNeoRename"]?.neoGlobalRestEndpointNonce) { return; }
        const now = Date.now();
