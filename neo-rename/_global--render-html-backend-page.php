...
function register_backend_page($get_query_param, \Closure $permission_callback, \Closure $html_output_callback) {
    if (!str_starts_with($get_query_param, "neo-")) { \NeoRename\NeoGlobal\throw_global_exception("GET parameter of the registered backend page must start with 'neo-' to avoid conflicts with other GET parameters."); }
    if (\NeoRename\NeoGlobal\query_param($get_query_param) === null) { return; }
    \NeoRename\NeoGlobal\add_action_hook("current_screen:0", function () use ($get_query_param, $permission_callback, $html_output_callback) {
