...
( function( $ ) {
	$( document ).ready( function() {
		var modalContentHtml          = <?php /* phpcs:ignoreFile */ echo json_encode( $modal_content_html ) ?>,
			modalHtml                 =
				'<div class="fs-modal fs-modal-developer-license-debug-mode fs-modal-developer-license-debug-mode-<?php /* phpcs:ignoreFile */ echo $unique_affix ?>">'
				...
				+ '			<button class="button button-primary button-submit-license-or-user-key"  tabindex="3"><?php /* phpcs:ignoreFile */ echo esc_js( $submit_button_text ) ?></button>'
				+ '		</div>'
				+ '	</div>'
				+ '</div>',
			$modal                             = $( modalHtml ),
            ...
            isDebugMode                        = <?php /* phpcs:ignoreFile */ echo $fs->is_data_debug_mode() ? 'true' : 'false' ?>;
		...
		function setDeveloperLicenseDebugMode( licenseOrUserKey ) {
            var data = {
                action             : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_action( 'set_data_debug_mode' ) ?>',
                security           : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_security( 'set_data_debug_mode' ) ?>',
            ...
            $.ajax( {
                ...
                data      : data,
                beforeSend: function () {
                    $debugLicenseLink.find('span').text( '<?php /* phpcs:ignoreFile */ echo $processing_text ?>' );
                    $submitKeyButton.text( '<?php /* phpcs:ignoreFile */ echo $processing_text ?>' );
                },
                ...
                error     : function () {
                    showError( <?php /* phpcs:ignoreFile */ echo json_encode( fs_text_inline( 'An unknown error has occurred.', 'unknown-error', $slug ) ) ?> );
                    resetButtons();
                }
            });
        }
		...
		function resetButtons() {
			enableSubmitButton();
			$submitKeyButton.text( <?php /* phpcs:ignoreFile */ echo json_encode( $submit_button_text ) ?> );
			$debugLicenseLink.find('span').text( <?php /* phpcs:ignoreFile */ echo json_encode( $debug_license_link_text ) ?> );
		}
