...
<script type="text/javascript">
	(function ($) {
		$(document).ready(function () {
			var contentHtml      = <?php /* phpcs:ignoreFile */ echo json_encode( $modal_content_html ); ?>,
			    modalHtml        =
				    '<div class="fs-modal fs-modal-license-key-resend <?php /* phpcs:ignoreFile */ echo $is_freemium ? 'fs-freemium' : 'fs-premium' ?>">'
				    + ' <div class="fs-modal-dialog">'
				    + '     <div class="fs-modal-header">'
				    + '         <h4><?php /* phpcs:ignoreFile */ echo esc_js( $send_button_text ) ?></h4>'
				    + '         <a href="#!" class="fs-close" tabindex="3" title="Close"><i class="dashicons dashicons-no" title="<?php /* phpcs:ignoreFile */ echo esc_js( fs_text_x_inline( 'Dismiss', 'as close a window', 'dismiss', $slug ) ) ?>"></i></a>'
				    + '     </div>'
			    ...
			    isFreemium       = <?php /* phpcs:ignoreFile */ echo json_encode( $is_freemium ) ?>,
			    userEmail        = <?php /* phpcs:ignoreFile */ echo json_encode( $email ) ?>,
			    moduleID         = '<?php /* phpcs:ignoreFile */ echo $fs->get_id() ?>',
			    isChild          = false;
			...
			function registerEventHandlers() {
				$('a.show-license-resend-modal-<?php /* phpcs:ignoreFile */ echo $fs->get_unique_affix() ?>').click(function (evt) {
					evt.preventDefault();
				...
				$modal.on('click', '.button', function (evt) {
					...
					$.ajax({
						...
						method    : 'POST',
						data      : {
							action     : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_action( 'resend_license_key' ) ?>',
							security   : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_security( 'resend_license_key' ) ?>',
							module_id  : moduleID,
							email      : email
						},
						beforeSend: function () {
							$sendButton.text('<?php /* phpcs:ignoreFile */ fs_esc_js_echo_inline( 'Sending license key', 'sending-license-key', $slug ) ?>...');
						},
			...
			function resetButton() {
				updateButtonState();
				$sendButton.text(<?php /* phpcs:ignoreFile */ echo json_encode($send_button_text) ?>);
			}
