...
(function( $ ) {
    $( document ).ready(function() {
        var modalContentHtml            = <?php /* phpcs:ignoreFile */ echo json_encode( $modal_content_html ) ?>,
            modalHtml                   =
                '<div class="fs-modal fs-modal-change-user fs-modal-change-user-<?php /* phpcs:ignoreFile */ echo $fs->get_unique_affix() ?>">'
                ...
                + '			<button class="button button-primary fs-user-change-button" tabindex="3"><?php /* phpcs:ignoreFile */ echo esc_js( $user_change_button_text ) ?></button>'
                + '		</div>'
                + '	</div>'
                + '</div>',
            $modal                      = $( modalHtml ),
        ...
        function registerEventHandlers() {
            ...
            $modal.on( 'click', '.fs-user-change-button', function ( evt ) {
                ...
                $.ajax( {
                    ...
                    method    : 'POST',
                    data      : {
                        action       : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_action( 'change_user' ) ?>',
                        security     : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_security( 'change_user' ) ?>',
                    ...
                    beforeSend: function () {
                        $userChangeButton
                            .text( '<?php /* phpcs:ignoreFile */ fs_esc_js_echo_inline( 'Changing user, please wait', 'changing-user-please-wait', $slug ) ?>...' )
        ...
        function resetUserChangeButton() {
            enableUserChangeButton();
            $userChangeButton.text( <?php /* phpcs:ignoreFile */ echo json_encode( $user_change_button_text ) ?> );
        }
