...
function echo_switch_for_option($option_name, $label_text = "", $only_enabled_if_pro = false, $js_function_on_change = null) {
    ...
    $is_checked = \NeoAlt\NeoGlobal\get_or_set_option($option_name);
    ...
    ?><neo-switch-neo-alt 
        ...
        onchange="return window.neoChangeSwitchForOptionNeoAlt(this, <?php echo esc_attr($js_function_on_change ?? 'null')?>)"
    ></neo-switch-neo-alt><?php
    \NeoAlt\NeoGlobal\add_action_hook("admin_footer", function () {
        \NeoAlt\NeoGlobal\enqueue_js("_global-web-component-neo-switch-for-option.js");
    });
}
