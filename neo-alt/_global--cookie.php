...
function cookie_set($name, $value, $expire = 0, $path = "/", $domain = "", $secure = false, $httponly = false) {
    ...
    $_COOKIE[$name] = $value;
    if (!$success) { \NeoAlt\NeoGlobal\global_warn_with_module_name("_global--cookie", "Failed to set cookie: " . $name); }
}

function cookie_delete($name, $path = "/", $domain = "", $secure = false, $httponly = false) {
    $success = setcookie($name, "", -1, $path, $domain, $secure, $httponly);
    unset($_COOKIE[$name]);
    if (!$success) { \NeoAlt\NeoGlobal\global_warn_with_module_name("_global--cookie", "Failed to delete cookie: " . $name); }
}
