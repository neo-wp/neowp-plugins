...
\NeoAlt\NeoGlobal\add_action_hook("neo_init", function () {
    \NeoAlt\NeoGlobal\register_rest_endpoint("/wp-json/neo/image-converter-source-neo-alt", "GET", fn () => \NeoAlt\NeoGlobal\current_user_can__global_endpoint__refresh_nonce(), function ($get_param) {
        ...
        if (!($max_size >= 1 && $max_size <= 10000 && $min_short_side >= 1 && $min_short_side <= 10000)) { \NeoAlt\NeoGlobal\throw_global_exception("Image converter source dimensions are invalid.", status_code: 400); }
        $decoded_image_url = \NeoAlt\NeoGlobal\percent_decode_invalid_utf8_url_bytes($image_url);
        $uploads_url_parts = wp_parse_url(\NeoAlt\NeoGlobal\uploads_url()); $image_url_parts = wp_parse_url($decoded_image_url);
        ...
        $add_source_candidate = function ($candidate_url, $candidate_path, $candidate_width, $candidate_height) use (&$source_candidates, $original_width, $original_height, $max_size, $min_short_side) {
            ...
            $source_candidates[] = ["url" => $candidate_url, "longestSide" => max($candidate_width, $candidate_height), "bytes" => \NeoAlt\NeoGlobal\fs_is_file($candidate_path) ? \NeoAlt\NeoGlobal\fs_filesize($candidate_path) : PHP_INT_MAX];
        };
        ...
        [$cached_image_paths, $interface_ok] = \NeoAlt\NeoGlobal\call_interface_func_implemented('\NeoAlt\NeoOptimize\interface_list_cached_images_20250302')($decoded_image_url);
        ...
        return ["imageUrl" => \NeoAlt\NeoGlobal\percent_encode_invalid_utf8_url_bytes($source_candidates[0]["url"] ?? $decoded_image_url)];
    });
});
