...
(function( $ ) {
	$( document ).ready(function() {
		var modalContentHtml = <?php /* phpcs:ignoreFile */ echo json_encode($modal_content_html); ?>,
			modalHtml =
				'<div class="fs-modal fs-modal-license-activation fs-modal-license-activation-<?php /* phpcs:ignoreFile */ echo $unique_affix ?>">'
				+ '	<div class="fs-modal-dialog">'
				+ '		<div class="fs-modal-header">'
				+ '		    <h4><?php /* phpcs:ignoreFile */ echo esc_js($header_title) ?></h4>'
				+ '         <a href="!#" class="fs-close"><i class="dashicons dashicons-no" title="<?php /* phpcs:ignoreFile */ echo esc_js( fs_text_x_inline( 'Dismiss', 'as close a window', 'dismiss', $slug ) ) ?>"></i></a>'
				+ '		</div>'
				+ '		<div class="fs-modal-body">'
				+ '			<div class="fs-modal-panel active">' + modalContentHtml + '</div>'
				+ '		</div>'
				+ '		<div class="fs-modal-footer">'
				+ '			<button class="button button-secondary button-close" tabindex="4"><?php /* phpcs:ignoreFile */ fs_esc_js_echo_inline( 'Cancel', 'cancel', $slug ) ?></button>'
				+ '			<button class="button button-primary button-activate-license"  tabindex="3"><?php /* phpcs:ignoreFile */ echo esc_js( $activate_button_text ) ?></button>'
				+ '		</div>'
				+ '	</div>'
				+ '</div>',
			$modal = $(modalHtml),
            ...
            isNetworkActivation             = <?php /* phpcs:ignoreFile */ echo $is_network_activation ? 'true' : 'false' ?>,
            isUserChangeSupported           = <?php /* phpcs:ignoreFile */ echo $is_user_change_supported ? 'true' : 'false' ?>,
        ...
        var
            ...
            $otherLicenseKey     = $modal.find( '#other_license_key_<?php /* phpcs:ignoreFile */ echo $unique_affix ?>' ),
            ...
            totalSites           = <?php /* phpcs:ignoreFile */ echo count( $sites_details ) ?>,
            singleBlogID         = null;
        ...
        var
            ...
            resetLoadingMode = function () {
                // Reset loading mode.
                $activateLicenseButton.text( <?php /* phpcs:ignoreFile */ echo json_encode( $activate_button_text ) ?> );
            ...
            afterLicenseUserDataLoaded = function () {
                if (
                    ...
                    otherLicenseOwnerID != <?php /* phpcs:ignoreFile */ echo $fs->is_registered() ? $fs->get_user()->id : 'null' ?>
                ) {
            ...
            fetchLicenseUserData = function () {
                ...
                $activateLicenseButton.html( '<?php /* phpcs:ignoreFile */ fs_esc_js_echo_inline( 'Please wait', 'please-wait', $slug ) ?>...' );
		...
		function registerEventHandlers() {
            ...
            $body.on( 'click', 'span.activate-license.<?php /* phpcs:ignoreFile */ echo $unique_affix ?> a, .activate-license-trigger.<?php /* phpcs:ignoreFile */ echo $unique_affix ?>', function (evt) {
				evt.preventDefault();
			...
			$modal.on('click', '.button-activate-license', function (evt) {
				...
				$.ajax({
                    ...
                    data: data,
					beforeSend: function () {
						$activateLicenseButton.text( '<?php /* phpcs:ignoreFile */ fs_esc_js_echo_inline( 'Activating license', 'activating-license', $slug ) ?>...' );
					},
					success: function( result ) {
        ...
        function toggleActivationOnAllSites() {
            ...
            var
                activateLicenseCheckboxLabel = '<?php /* phpcs:ignoreFile */ fs_esc_js_echo_inline( 'Choose up to %s site(s) to activate the license on.', 'choose-up-to-n-sites-to-activate-the-license-on', $slug ) ?>';
        ...
        function resetActivateLicenseCheckboxLabel() {
            var activateLicenseCheckboxLabel = '<?php /* phpcs:ignoreFile */ fs_esc_js_echo_inline( 'Activate license on all sites in the network.', 'activate-license-on-all-sites-in-the-network', $slug ) ?>';
		...
		function showModal( evt ) {
            ...
            <?php /* phpcs:ignoreFile */ if ( $fs->apply_filters( 'enable_per_site_activation', true ) ) : ?>
