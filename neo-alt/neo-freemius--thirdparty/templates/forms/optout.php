...
?>
<div id="<?php /* phpcs:ignoreFile */ echo esc_attr( $form_id ) ?>"
     class="fs-modal fs-modal-opt-out"
     data-plugin-id="<?php /* phpcs:ignoreFile */ echo esc_attr( $fs->get_id() ) ?>"
     data-action="<?php /* phpcs:ignoreFile */ echo esc_attr( $fs->get_ajax_action( $ajax_action ) ) ?>"
     data-security="<?php /* phpcs:ignoreFile */ echo esc_attr(  $fs->get_ajax_security( $ajax_action ) ) ?>"
     style="display: none">
    <div class="fs-modal-dialog">
        <div class="fs-modal-header">
            <h4><?php /* phpcs:ignoreFile */ echo esc_html( $opt_out_text ) ?></h4>
        ...
        <div class="fs-opt-out-permissions">
            <div class="fs-modal-body">
                ...
                <?php /* phpcs:ignoreFile */ foreach ( $permission_groups as $i => $permission_group ) : ?>
                    <?php /* phpcs:ignoreFile */ $permission_manager->render_permissions_group( $permission_group ) ?>
                    <?php /* phpcs:ignoreFile */ if ( $i < count( $permission_groups ) - 1 ) : ?><hr><?php /* phpcs:ignoreFile */ endif ?>
                <?php /* phpcs:ignoreFile */ endforeach ?>
                </div>
            </div>
            <div class="fs-modal-footer">
                <button class="button button-primary button-close" tabindex="1"><?php /* phpcs:ignoreFile */ echo $fs->esc_html_inline( 'Done', 'done' ) ?></button>
            </div>
        </div>
        <?php /* phpcs:ignoreFile */ foreach ( $permission_groups as $i => $permission_group ) : ?>
            <?php /* phpcs:ignoreFile */ if ( ! empty( $permission_group[ 'prompt' ] ) ) : ?>
                <div class="fs-<?php /* phpcs:ignoreFile */ echo esc_attr( $permission_group[ 'id' ] ) ?>-opt-out fs-opt-out-disclaimer" data-group-id="<?php /* phpcs:ignoreFile */ echo esc_attr( $permission_group[ 'id' ] ) ?>" style="display: none">
                    <div class="fs-modal-body">
                        <div class="fs-modal-panel active">
                            <div class="notice notice-error inline opt-out-error-message"><p></p></div>
                            <?php /* phpcs:ignoreFile */ foreach ( $permission_group[ 'prompt' ] as $p ) : ?>
                                <?php /* phpcs:ignoreFile */ // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
                                <p><?php /* phpcs:ignoreFile */ echo $p ?></p>
                            <?php /* phpcs:ignoreFile */ endforeach ?>
                        </div>
                    </div>
                    <div class="fs-modal-footer">
                        <a class="fs-opt-out-button" tabindex="2" href="#"><?php /* phpcs:ignoreFile */ echo esc_html( $opt_out_text ) ?></a>
                        <button class="button button-primary fs-opt-out-cancel-button" tabindex="1"><?php /* phpcs:ignoreFile */ echo esc_html( $permission_group[ 'prompt_cancel_label' ] ) ?></button>
                    </div>
                </div>
...
<?php /* phpcs:ignoreFile */ $permission_manager->require_permissions_js( false ) ?>

<script type="text/javascript">
	(function( $ ) {
		$( document ).ready(function() {
            FS.OptOut(
                <?php /* phpcs:ignoreFile */ echo wp_json_encode( $fs->get_id() ) ?>,
                <?php /* phpcs:ignoreFile */ echo wp_json_encode( $slug ) ?>,
                <?php /* phpcs:ignoreFile */ echo wp_json_encode( $fs->get_module_type() ) ?>,
                <?php /* phpcs:ignoreFile */ echo $fs->is_registered( true ) ? 'true' : 'false' ?>,
                <?php /* phpcs:ignoreFile */ echo $fs->is_tracking_allowed() ? 'true' : 'false' ?>,
                <?php /* phpcs:ignoreFile */ echo wp_json_encode( $reconnect_url ) ?>
            );
		});
	})( jQuery );
</script>
