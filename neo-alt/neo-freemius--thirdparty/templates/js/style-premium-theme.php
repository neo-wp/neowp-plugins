...
<script type="text/javascript">
	(function ($) {
		...
		var $theme             = $('#<?php /* phpcs:ignoreFile */ echo $premium_slug ?>-name').parents('.theme'),
		    addPremiumMetadata = function (firstCall) {
			    if (!firstCall) {
				    ...
				    $theme = $('#<?php /* phpcs:ignoreFile */ echo $premium_slug ?>-name').parents('.theme');
			    }
			    ...
			    if (0 === $theme.find('.fs-premium-theme-badge-container').length) {
				    ...
				    $themeBadgeContainer.append( '<div class="fs-badge fs-premium-theme-badge">' + <?php /* phpcs:ignoreFile */ echo json_encode( $fs->get_text_inline( 'Premium', 'premium' ) ) ?> + '</div>' );

				    <?php /* phpcs:ignoreFile */ if ( $fs->is_beta() ) : ?>
                    $themeBadgeContainer.append( '<div class="fs-badge fs-beta-theme-badge">' + <?php /* phpcs:ignoreFile */ echo json_encode( $fs->get_text_inline( 'Beta', 'beta' ) ) ?> + '</div>' );
