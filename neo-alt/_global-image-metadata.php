...
function get_image_metadata(string $svg_content, bool $only_read_header = false) {
    ...
    if ($start_pos === false) { \NeoAlt\NeoGlobal\global_warn_with_module_name("_global-image-metadata", "Invalid metadata in SVG: Start tag not found"); return false; }
    $json_start = $start_pos + strlen($start_tag);
    $end_pos = strpos($svg_content, $end_tag, $json_start);
    if ($end_pos === false && !$only_read_header) { \NeoAlt\NeoGlobal\global_warn_with_module_name("_global-image-metadata", "Invalid metadata in SVG: End tag not found"); return false; }
    $raw = ($end_pos === false) ? substr($svg_content, $json_start) : substr($svg_content, $json_start, $end_pos - $json_start);
    $cleaned = \NeoAlt\NeoGlobal\preg_replace_better("/^\s*<!--\s*/", "", $raw ?? "");
    $cleaned = \NeoAlt\NeoGlobal\preg_replace_better("/\s*-->\s*$/", "", $cleaned ?? "");
    if ($only_read_header) {
        \NeoAlt\NeoGlobal\preg_match_better("/\"headerEnd\"\s*:\s*null/", $cleaned, $header_end_matches, PREG_OFFSET_CAPTURE);
        if (count($header_end_matches) === 0) { \NeoAlt\NeoGlobal\global_warn_with_module_name("_global-image-metadata", "Invalid metadata in SVG: No headerEnd"); return false; }
        $cut_pos = $header_end_matches[0][1] + strlen($header_end_matches[0][0]);
        $cleaned = substr($cleaned, 0, $cut_pos) . "}";
    }
    $metadata = \NeoAlt\NeoGlobal\json_decode_better($cleaned, suppress_error: true);
    if ($metadata === false) { \NeoAlt\NeoGlobal\global_warn_with_module_name("_global-image-metadata", "Invalid metadata in SVG JSON"); return false; }
    return $metadata;
}

function get_image_metadata_by_path($file_path) {
    $file_content_first_4_kb = \NeoAlt\NeoGlobal\fs_file_get_contents($file_path, false, null, 0, 4096);
...
function get_image_metadata_by_post_id($post_id) {
    ...
    if (!\NeoAlt\NeoGlobal\fs_file_exists($attached_file_path)) { return null; }
...
function is_neodraw_image_path($file_path) {
    ...
    $file_content_first_4_kb = \NeoAlt\NeoGlobal\fs_file_get_contents($file_path, false, offset: 0, length: 4096);
...
function is_neodraw_image_post($image_post) {
    ...
    if (!\NeoAlt\NeoGlobal\fs_file_exists($attached_file)) { return false; }
