...
const sendNeoLogToEndpoint = async (type, message) => {
    try {
        if (!fetchEndpoint.nonce && !window["neoGlobalEnqueueBackendNeoAlt"]?.neoGlobalRestEndpointNonce && !window["neoGlobalEnqueueFrontendNeoAlt"]?.neoGlobalRestEndpointNonce) { return; }
        const now = Date.now();
