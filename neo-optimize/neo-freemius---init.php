...
function freemius_product_id() {
    ...
    if (isset(\NeoOptimize\NeoGlobal\val("custom")["license_settings"]) && (\NeoOptimize\NeoGlobal\val("custom")["license_settings"]["freemius_product_id"] ?? null) !== $hardcoded_value) { \NeoOptimize\NeoGlobal\admin_error("Freemius product ID mismatch: " . (\NeoOptimize\NeoGlobal\val("custom")["license_settings"]["freemius_product_id"] ?? "[MISSING]") . " != " . $hardcoded_value . ". Update hardcoded value in " . basename(__FILE__) . "."); }
    return $hardcoded_value;
}
function freemius_product_public_key() {
    $hardcoded_value = "pk_01093f748756a4db76d7003ade4ab";
    if (isset(\NeoOptimize\NeoGlobal\val("custom")["license_settings"]) && (\NeoOptimize\NeoGlobal\val("custom")["license_settings"]["freemius_product_public_key"] ?? null) !== $hardcoded_value) { \NeoOptimize\NeoGlobal\admin_error("Freemius public key mismatch: " . (\NeoOptimize\NeoGlobal\val("custom")["license_settings"]["freemius_product_public_key"] ?? "[MISSING]") . " != " . $hardcoded_value . ". Update hardcoded value in " . basename(__FILE__) . "."); }
    return $hardcoded_value;
}
function freemius_slug_free() {
    $hardcoded_value = "neo-media";
    if (isset(\NeoOptimize\NeoGlobal\val("custom")["license_settings"]) && (\NeoOptimize\NeoGlobal\val("custom")["license_settings"]["freemius_slug_free"] ?? null) !== $hardcoded_value) { \NeoOptimize\NeoGlobal\admin_error("Freemius free slug mismatch: " . (\NeoOptimize\NeoGlobal\val("custom")["license_settings"]["freemius_slug_free"] ?? "[MISSING]") . " != " . $hardcoded_value . ". Update hardcoded value in " . basename(__FILE__) . "."); }
    return $hardcoded_value;
}
function freemius_slug_pro()  {
    $hardcoded_value = "neo-media-pro";
    if (isset(\NeoOptimize\NeoGlobal\val("custom")["license_settings"]) && (\NeoOptimize\NeoGlobal\val("custom")["license_settings"]["freemius_slug_pro"] ?? null) !== $hardcoded_value) { \NeoOptimize\NeoGlobal\admin_error("Freemius pro slug mismatch: " . (\NeoOptimize\NeoGlobal\val("custom")["license_settings"]["freemius_slug_pro"] ?? "[MISSING]") . " != " . $hardcoded_value . ". Update hardcoded value in " . basename(__FILE__) . "."); }
    return $hardcoded_value;
}
...
function init_freemius_once_and_get_instance() {
    static $neo_fs = null; if ($neo_fs === null) {
        if (get_option("fs_accounts")) {
            ...
            $freemius_main_file = plugin_basename(\NeoOptimize\NeoGlobal\plugin_entry_file_path()); foreach (\NeoOptimize\NeoEntrypoint\get_neo_active_plugins() as $active_plugin) { if ($active_plugin["slug"] === "neo-media") { $freemius_main_file = plugin_basename($active_plugin["plugin_entry_file_path"]); break; } }
        ...
        [$freemius_secret_key, $interface_ok] = \NeoOptimize\NeoGlobal\call_interface_func_implemented('\NeoOptimize\NeoDebug\interface_set_up_debug_mode_and_get_freemius_debug_secret_key_20260205')(freemius_slug_free(), freemius_slug_pro()); if (!$interface_ok) { $freemius_secret_key = null; }

        (fn () => require_once(\NeoOptimize\NeoGlobal\plugin_path() . "/" . "neo-freemius--thirdparty/start.php"))();
        $neo_fs = fs_dynamic_init([
            ...
            "has_paid_plans"      => true,
            "is_premium"          => false,
            "is_premium_only"     => \NeoOptimize\NeoGlobal\plugin_slug() === "neo-media" && wp_strip_all_tags(wp_unslash($_GET["require_license"] ?? "")) !== "false", /* phpcs:ignore WordPress.Security.NonceVerification.Recommended */ /* There is no nonce to verify */
            "trial" => [
                "days"               => 14,
                "is_require_payment" => true,
            ],
            "menu" => [
                "slug"    => \NeoOptimize\NeoGlobal\plugin_settings_page_slug(),
                "contact" => false,
                "support" => false,
            ],
        ...
        $neo_fs->add_filter("plugin_icon", fn () => \NeoOptimize\NeoGlobal\plugin_path_no_symlink_follow() . "/img/_global--logo-icon.svg");
        ...
        \NeoOptimize\NeoGlobal\add_filter_hook($neo_fs->get_action_tag("is_submenu_visible"), function ($is_visible, $submenu_id) {  return $is_visible; });
        ...
        foreach (\NeoOptimize\NeoEntrypoint\get_neo_active_plugins() as $active_plugin) {
            ...
            \NeoOptimize\NeoGlobal\add_filter_hook("plugin_action_links_" . plugin_basename($active_plugin["plugin_entry_file_path"]), function ($actions, $plugin_file) use ($neo_fs) { return $neo_fs->_modify_plugin_action_links_hook($actions, $plugin_file); });
        }
    }
    return $neo_fs;
}
\NeoOptimize\NeoGlobal\add_action_hook("neo_init", function () { init_freemius_once_and_get_instance(); });
\NeoOptimize\NeoGlobal\add_action_hook("neo_init", function () {
    if (!(\NeoOptimize\NeoGlobal\query_param("neo-freemius--suppress-opt-in") === "true")) { return; }
...
function interface_freemius_activate_hook_20260210  ($plugin_slug, $plugin_edition, $plugin_version) { static $first_run = true; if (!$first_run) { return; } $first_run = false; \NeoOptimize\NeoGlobal\global_log_with_module_name("neo-freemius", "Running freemius activate hook");   init_freemius_once_and_get_instance()->remove_sticky("trial_promotion"); init_freemius_once_and_get_instance()->_activate_plugin_event_hook(); }
function interface_freemius_deactivate_hook_20260210($plugin_slug, $plugin_edition, $plugin_version) { static $first_run = true; if (!$first_run) { return; } $first_run = false; \NeoOptimize\NeoGlobal\global_log_with_module_name("neo-freemius", "Running freemius deactivate hook"); init_freemius_once_and_get_instance()->remove_sticky("trial_promotion"); init_freemius_once_and_get_instance()->_deactivate_plugin_hook(); }
function interface_freemius_uninstall_hook_20260210 ($plugin_slug, $plugin_edition, $plugin_version) { static $first_run = true; if (!$first_run) { return; } $first_run = false; \NeoOptimize\NeoGlobal\global_log_with_module_name("neo-freemius", "Running freemius uninstall hook");  init_freemius_once_and_get_instance()->_uninstall_plugin_hook(); }
...
function remove_all_license_data() {
    ...
    \NeoOptimize\NeoGlobal\call_interface_func_implemented('\NeoOptimize\NeoGlobal\interface_all_bundles_clear_license_cache_20260131', throw_if_interface_not_ok: true)();
}
\NeoOptimize\NeoGlobal\register_rest_endpoint("/wp-json/neo/remove-freemius-license-data", "POST", fn () => \NeoOptimize\NeoGlobal\current_user_can__neo_freemius__delete_license_data(), function ($get_param) {
...
\NeoOptimize\NeoGlobal\add_action_hook("neo_init", function () {
    \NeoOptimize\NeoGlobal\call_interface_func_implemented('\NeoOptimize\NeoReset\interface_register_neo_reset_action_20260410')(
        id: "neo-freemius--delete-license",  button_text: \NeoOptimize\NeoGlobal\neo__("Delete all license data", "Alle Lizenzdaten löschen"), confirm_title: \NeoOptimize\NeoGlobal\neo__("Reset license?", "Lizenz zurücksetzen?"), confirm_text: \NeoOptimize\NeoGlobal\neo__("Are you sure you want to remove all license data? You will need to enter the license key again.", "Möchtest du wirklich alle Lizenzdaten entfernen? Der Lizenzschlüssel muss erneut eingegeben werden."),
...
(function () {
    foreach (\NeoOptimize\NeoEntrypoint\get_neo_active_plugins() as $active_plugin) {
        \NeoOptimize\NeoGlobal\add_filter_hook("plugin_action_links_" . plugin_basename($active_plugin["plugin_entry_file_path"]) . ":11", function ($actions, $plugin_file, $plugin_data, $context) {
            if (!str_starts_with($plugin_file, "neo-")) { return $actions; }

            $premium_url = interface_freemius_pricing_page_url_20260131() ?? ("https://" . \NeoOptimize\NeoGlobal\option__neo_wp_com() . "/pricing/?ref=wp-backend-get-premium#pricing-table");
            ...
            $actions["upgrade"] = "<a href=\"" . esc_url($premium_url) . "\"" . $premium_url_target . " style=\"font-weight: bold; color: #a03b1f;\">" . \NeoOptimize\NeoGlobal\neo__("Get Premium", "Premium holen") . "</a>";
