...
const sendNeoLogToEndpoint = async (type, message) => {
    try {
        if (!fetchEndpoint.nonce && !window["neoGlobalEnqueueBackendNeoOptimize"]?.neoGlobalRestEndpointNonce && !window["neoGlobalEnqueueFrontendNeoOptimize"]?.neoGlobalRestEndpointNonce) { return; }
        const now = Date.now();
