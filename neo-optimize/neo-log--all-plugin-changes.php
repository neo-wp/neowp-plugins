...
\NeoOptimize\NeoGlobal\add_action_hook("current_screen", function () {
    $log_data = ["siteHost" => \NeoOptimize\NeoGlobal\site_host(), "activePlugins" => \NeoOptimize\NeoEntrypoint\get_neo_active_plugins(), "wpVersion" => get_bloginfo("version"), "phpVersion" => phpversion(), "plugins" => get_option("active_plugins"), "wpContentPath" => WP_CONTENT_DIR]; /* Usage of WP_CONTENT_DIR is OK here because it is only meant for debugging when the user provides us their log files #suppressLinterWporgDirectoryConstantCheck */

    $global_log_path = \NeoOptimize\NeoGlobal\cache_path("neo-log") . "/neo-log--plugin-changes.log";
    $get_last_non_empty_line = function ($file_path) { return !\NeoOptimize\NeoGlobal\fs_file_exists($file_path) ? null : ((($file = new \SplFileObject($file_path, "r")) && $file->seek(PHP_INT_MAX) === null && ($line = trim($file->current())) !== "") ? $line : ((($file->key() > 0 && $file->seek($file->key() - 1) === null && ($line = trim($file->current())) !== "")) ? $line : (($file->seek(0) === null && ($line = trim($file->current())) !== "") ? $line : null))); };
    $last_global_log_line = \NeoOptimize\NeoGlobal\json_decode_better($get_last_non_empty_line($global_log_path) ?? "{}", suppress_error: true); $last_global_log_message = $last_global_log_line === false ? null : ($last_global_log_line["message"] ?? null);
    $last_global_log_data = is_string($last_global_log_message) ? \NeoOptimize\NeoGlobal\json_decode_better($last_global_log_message, suppress_error: true) : false;
    ...
    $log_message = \NeoOptimize\NeoGlobal\json_encode_better($log_data);
    \NeoOptimize\NeoGlobal\global_log_with_module_name("neo-log", $log_message, log_source: "neo-log--plugin-changes");
});
