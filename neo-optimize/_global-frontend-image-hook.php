...
\NeoOptimize\NeoGlobal\val("neo_global__frontend_image_hook_callbacks", []);
function frontend_image_hook_register_callback($callback) {
    \NeoOptimize\NeoGlobal\val("neo_global__frontend_image_hook_callbacks")[] = $callback;
}
...
function frontend_image_hook_filter_html($html) {
    ...
    $process_image = function ($img_attributes) {
        ...
        if (!\NeoOptimize\NeoGlobal\is_url_internal($get_attr("src"))) { return []; }

        try {
            $image_file_path = \NeoOptimize\NeoGlobal\get_path_from_internal_url($get_attr("src"));
        } catch (\Throwable $e) { return []; }

        if (!\NeoOptimize\NeoGlobal\fs_file_exists($image_file_path)) { return []; }

        foreach (\NeoOptimize\NeoGlobal\val("neo_global__frontend_image_hook_callbacks") as $callback) {
            try {
                $callback($get_attr, $set_attr, $image_file_path);
            } catch (\Throwable $e) { \NeoOptimize\NeoGlobal\global_warn_with_module_name("_global-frontend-image-hook", $e->getMessage() . " - Failed to process image: " . $get_attr("src")); }
        }
    ...
    $split_css_url_match = function ($img_url) {
        ...
        for ($i = 0; $i < strlen($img_url); $i++) {
            ...
            if (\NeoOptimize\NeoGlobal\preg_match_all_better("/[&?](neo-global--processed-[a-z0-9-]+)=([^&#)]*)/i", $suffix, $processed_param_matches, PREG_SET_ORDER)) {
                foreach ($processed_param_matches as $processed_param_match) {
                    $img_url = \NeoOptimize\NeoGlobal\add_query_param($img_url, $processed_param_match[1], urldecode($processed_param_match[2]));
                }
                $suffix = \NeoOptimize\NeoGlobal\preg_replace_better("/[&?]neo-global--processed-[a-z0-9-]+=[^&#)]*/i", "", $suffix);
            }
    ...
    $html = \NeoOptimize\NeoGlobal\preg_replace_callback_better($img_tag_or_url_regex, function ($matches) use ($html, $process_image, $img_url_regex_raw, $split_css_url_match) {
        if (($matches["img_tag"][1] ?? -1) !== -1) {
            $img_tag = $matches["img_tag"][0];

            if (\NeoOptimize\NeoGlobal\preg_match_better("/(?:^|\s)[a-zA-Z0-9_:-]+\s*=\s*\\\\[\"']/", $img_tag)) { return $img_tag; }

            if (str_contains($img_tag, "data-neo-global--processed-neo-optimize")) { return $img_tag; }

            $attr_regex_pattern = '/(?<spacing_before_attr>\s+)(?<key>[a-zA-Z0-9_\-:]+)(?:(?<equals>=)(?:"(?<value1>[^"]*)"|\'(?<value2>[^\']*)\'|(?<value3>[^ \t\r\n"\'>]*)))?/';
            $attributes = [];
            if (\NeoOptimize\NeoGlobal\preg_match_all_better($attr_regex_pattern, $img_tag, $matches, PREG_SET_ORDER)) {
                foreach ($matches as $match) {
                    $attr_key   = $match["key"];
                    $attr_value = ($match["value1"] ?? "") ?: ($match["value2"] ?? "") ?: ($match["value3"] ?? "") ?: "";
                    if (isset($attributes[$attr_key])) { \NeoOptimize\NeoGlobal\global_warn_with_module_name("_global-frontend-image-hook", "Duplicate attribute in image tag: $attr_key in $img_tag. It was deduplicated."); }
            ...
            foreach ($attributes as $attr_key => $attr_value) {
                ...
                $attributes[$attr_key] = \NeoOptimize\NeoGlobal\preg_replace_callback_better("/$img_url_regex_raw/i", function ($match) use (&$attributes_before, &$process_image, &$attr_key, &$attribute_changes, &$attribute_changes_remembered_for_attribute_key, &$split_css_url_match) {
            ...
            $attributes["data-neo-global--processed-neo-optimize"] = "true";
            ...
            $new_img_tag = \NeoOptimize\NeoGlobal\preg_replace_better('/\s*\/?\s*>$/', $additional_attributes_string . "$0", $img_tag);

            $new_img_tag = \NeoOptimize\NeoGlobal\preg_replace_callback_better($attr_regex_pattern, function ($match) use (&$attributes, &$attribute_changes) {
            ...
            if (\NeoOptimize\NeoGlobal\preg_match_better('/href\s*=\s*["\']$/i', substr($html, max(0, $matches["img_url"][1] - 20), min(20, $matches["img_url"][1])))) { return $matches["img_url"][0]; }
            [$img_url, $img_url_suffix] = $split_css_url_match($matches["img_url"][0]);
            if (str_contains($img_url, "neo-global--processed-neo-optimize=true")) { return $img_url . $img_url_suffix; }
            if (!\NeoOptimize\NeoGlobal\is_url_internal($img_url)) { return $img_url . $img_url_suffix; }

            $img_url = $process_image(["src" => $img_url])["src"] ?? $img_url;

            $img_url = \NeoOptimize\NeoGlobal\add_query_param($img_url, "neo-global--processed-neo-optimize", "true");
            return $img_url . $img_url_suffix;
        }
    }, $html, flags: PREG_OFFSET_CAPTURE);
    return $html;
}
\NeoOptimize\NeoGlobal\add_filter_hook(
    "the_content",
