...
function json_encode_better($content_object, $pretty_print = false) {
    try { $json_encoded = json_encode($content_object, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR | ($pretty_print ? JSON_PRETTY_PRINT : 0)); }
        ...
        if (\function_exists(__NAMESPACE__ . "\\global_warn")) { \NeoOptimize\NeoGlobal\global_warn_with_module_name("_global--better-json-functions", "Invalid UTF-8 was replaced while encoding JSON."); }
...
function json_decode_better($json_string, $suppress_error = false) {
    ...
    else if (\NeoOptimize\NeoGlobal\preg_match_better('/(-?(?:\d+\.\d+|\d+)(?:e[+\-]?\d+))$/i', $json_string, $matches)){ $parsed_json = json_decode($matches[1]);$json_start_index = $length - strlen($matches[1]);}
    else if (\NeoOptimize\NeoGlobal\preg_match_better('/(-?\d+\.\d+)$/',$json_string, $matches)){ $parsed_json = json_decode($matches[1]);$json_start_index = $length - strlen($matches[1]);}
    else if (\NeoOptimize\NeoGlobal\preg_match_better('/(-?\d+)$/',$json_string, $matches)){ $parsed_json = json_decode($matches[1]);$json_start_index = $length - strlen($matches[1]);}
    else {
        for ($i = 0; $i < $length; $i++) {
    ...
    if ($json_start_index === -1) { \NeoOptimize\NeoGlobal\throw_global_exception("JSON Decoding Error. Tried to decode: " . $json_string); }
    $php_warning_string = substr($json_string, 0, $json_start_index);
    if ($php_warning_string !== "") { \NeoOptimize\NeoGlobal\global_warn_with_module_name("_global--better-json-functions", "Warning from PHP while decoding JSON: $php_warning_string"); }
    return $parsed_json;
}
