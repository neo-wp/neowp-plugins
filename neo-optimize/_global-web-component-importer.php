...
function get_web_component_names_for_importer() {
    $web_components = [];
    foreach (\NeoOptimize\NeoGlobal\fs_glob(\NeoOptimize\NeoGlobal\plugin_path() . "/_global-web-component-*.js") ?: [] as $file) {
        $file = basename($file);
        ...
        $web_component_name .= "-neo-optimize";
...
\NeoOptimize\NeoGlobal\callback_before_enqueue_js_variables_backend_or_frontend(function () {
    if (!is_admin()) { return; }
    \NeoOptimize\NeoGlobal\enqueue_js_variable_backend("neoGlobalWebComponentImporterWebComponents", get_web_component_names_for_importer());
});
...
\NeoOptimize\NeoGlobal\add_action_hook("current_screen", function () {
    foreach (get_web_component_names_for_importer() as $web_component_name) {
        \NeoOptimize\NeoGlobal\add_action_hook("admin_head", function () use ($web_component_name) {
            ?>
            <?php \NeoOptimize\NeoGlobal\backend_page_style_tag_start(["id" => \wp_specialchars_decode(esc_attr("neo-web-component-hide-neo-optimize-" . $web_component_name . "-inline-css"), ENT_QUOTES)]); ?>
                <?php echo esc_attr($web_component_name) ?> { <?php echo $web_component_name === "neo-info-tooltip-neo-optimize" ? "display: none;" : "visibility: hidden" ?> }
            <?php \NeoOptimize\NeoGlobal\backend_page_style_tag_end(); ?><?php
        });
    }
    \NeoOptimize\NeoGlobal\enqueue_js("_global-web-component-importer.js");
});
