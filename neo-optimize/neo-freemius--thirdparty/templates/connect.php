...
<div id="fs_theme_connect_wrapper">
	...
	?>
	<div id="fs_connect"
	     class="wrap<?php /* phpcs:ignoreFile */ if ( ! fs_is_network_admin() && ( ! $fs->is_enable_anonymous() || $is_pending_activation || $require_license_key ) ) {
		     echo ' fs-anonymous-disabled';
	     } ?><?php /* phpcs:ignoreFile */ echo $require_license_key ? ' require-license-key' : '' ?>">
        <div class="fs-header">
		...
		<div class="fs-content">
            <?php /* phpcs:ignoreFile */ $fs->do_action( 'connect/before_message', $activation_state ) ?>

			<?php /* phpcs:ignoreFile */ if ( ! empty( $error ) ) : ?>
				<div class="fs-error"><?php /* phpcs:ignoreFile */ echo $fs->apply_filters( 'connect_error_esc_html', esc_html( $error ) ) ?></div>
			...
			<?php /* phpcs:ignoreFile */ if ( $require_license_key ) : ?>
				<div class="fs-license-key-container">
					<input id="fs_license_key" name="fs_key" type="text" required maxlength="<?php /* phpcs:ignoreFile */ echo $fs->apply_filters('license_key_maxlength', 32) ?>"
					       placeholder="<?php /* phpcs:ignoreFile */ fs_esc_attr_echo_inline( 'License key', 'license-key', $slug ) ?>" tabindex="1"/>
					<i class="dashicons dashicons-admin-network"></i>
					<a class="show-license-resend-modal show-license-resend-modal-<?php /* phpcs:ignoreFile */ echo $fs->get_unique_affix() ?>"
					   href="#"><?php /* phpcs:ignoreFile */ fs_esc_html_echo_inline( "Can't find your license key?", 'cant-find-license-key', $slug ); ?></a>
				</div>
                ...
                <div id="fs_marketing_optin">
                    <span class="fs-message"><?php /* phpcs:ignoreFile */ fs_echo_inline( "Please let us know if you'd like us to contact you for security & feature updates, educational content, and occasional offers:", 'contact-for-updates' ) ?></span>
                    <div class="fs-input-container">
                        <label>
                            <input type="radio" name="allow-marketing" value="true" tabindex="1" />
                            <span class="fs-input-label"><?php /* phpcs:ignoreFile */ echo $send_updates_text ?></span>
                        </label>
                        <label>
                            <input type="radio" name="allow-marketing" value="false" tabindex="1" />
                            <span class="fs-input-label"><?php /* phpcs:ignoreFile */ echo $do_not_send_updates_text ?></span>
                        </label>
                    </div>
                </div>
                <div id="fs_orphan_license_message">
                    <span class="fs-message"><?php /* phpcs:ignoreFile */ fs_echo_inline( "A user has not yet been associated with the license, which is necessary to prevent unauthorized activation. To assign the license to your user, you agree to share your WordPress user's full name and email address." ) ?></span>
                </div>
			<?php /* phpcs:ignoreFile */ endif ?>
			<?php /* phpcs:ignoreFile */ if ( $is_network_level_activation ) : ?>
            <?php /* phpcs:ignoreFile */
                $vars = array(
                    'id'                  => $fs->get_id(),
                    'sites'               => $sites,
            ...
            <?php /* phpcs:ignoreFile */ $fs->do_action( 'connect/after_message', $activation_state ) ?>
		</div>
		<div class="fs-actions">
            <?php /* phpcs:ignoreFile */ $fs->do_action( 'connect/before_actions', $activation_state ) ?>

			<?php /* phpcs:ignoreFile */ if ( $fs->is_enable_anonymous() && ! $is_pending_activation && ( ! $require_license_key || $is_network_upgrade_mode ) ) : ?>
				<a id="skip_activation" href="<?php /* phpcs:ignoreFile */ echo fs_nonce_url( $fs->_get_admin_page_url( '', array( 'fs_action' => $fs->get_unique_affix() . '_skip_activation' ), $is_network_level_activation ), $fs->get_unique_affix() . '_skip_activation' ) ?>"
				   class="button button-secondary" tabindex="2"><?php /* phpcs:ignoreFile */ fs_esc_html_echo_x_inline( 'Skip', 'verb', 'skip', $slug ) ?></a>
			<?php /* phpcs:ignoreFile */ endif ?>
			<?php /* phpcs:ignoreFile */ if ( $is_network_level_activation && $fs->apply_filters( 'show_delegation_option', true ) ) : ?>
				<a id="delegate_to_site_admins" class="fs-tooltip-trigger <?php /* phpcs:ignoreFile */ echo is_rtl() ? ' rtl' : '' ?>" href="<?php /* phpcs:ignoreFile */ echo fs_nonce_url( $fs->_get_admin_page_url( '', array( 'fs_action' => $fs->get_unique_affix() . '_delegate_activation' ) ), $fs->get_unique_affix() . '_delegate_activation' ) ?>"><?php /* phpcs:ignoreFile */ fs_esc_html_echo_inline( 'Delegate to Site Admins', 'delegate-to-site-admins', $slug ) ?><span class="fs-tooltip"><?php /* phpcs:ignoreFile */ fs_esc_html_echo_inline( 'If you click it, this decision will be delegated to the sites administrators.', 'delegate-sites-tooltip', $slug ) ?></span></a>
			<?php /* phpcs:ignoreFile */ endif ?>
			<?php /* phpcs:ignoreFile */ if ( $activate_with_current_user ) : ?>
				<form action="" method="POST">
					<input type="hidden" name="fs_action"
					       value="<?php /* phpcs:ignoreFile */ echo esc_attr( $fs->get_unique_affix() . '_activate_existing' ) ?>">
					<?php /* phpcs:ignoreFile */ wp_nonce_field( $fs->get_unique_affix() . '_activate_existing' ) ?>
					...
					<button class="button button-primary" tabindex="1"
					        type="submit"><?php /* phpcs:ignoreFile */ echo esc_html( $button_label ) ?></button>
				</form>
			<?php /* phpcs:ignoreFile */ else : ?>
				<form method="post" action="<?php /* phpcs:ignoreFile */ echo WP_FS__ADDRESS ?>/action/service/user/install/">
					<?php /* phpcs:ignoreFile */ unset( $optin_params['sites']); ?>
					<?php /* phpcs:ignoreFile */ foreach ( $optin_params as $name => $value ) : ?>
						<input type="hidden" name="<?php /* phpcs:ignoreFile */ echo esc_attr( $name ) ?>" value="<?php /* phpcs:ignoreFile */ echo esc_attr( $value ) ?>">
					<?php /* phpcs:ignoreFile */ endforeach ?>
					<input type="hidden" name="is_extensions_tracking_allowed" value="1">
                    <input type="hidden" name="is_diagnostic_tracking_allowed" value="1">
					<button class="button button-primary" tabindex="1"
					        type="submit"<?php /* phpcs:ignoreFile */ if ( $require_license_key ) {
						echo ' disabled="disabled"';
					} ?>><?php /* phpcs:ignoreFile */ echo esc_html( $button_label ) ?></button>
				</form>
			<?php /* phpcs:ignoreFile */ endif ?>
            <?php /* phpcs:ignoreFile */ if ( $require_license_key ) : ?>
                <a id="license_issues_link"
                   href="<?php /* phpcs:ignoreFile */ echo esc_url( $fs->apply_filters( 'known_license_issues_url', 'https://freemius.com/help/documentation/wordpress-sdk/license-activation-issues/' ) ) ?>"
                   target="_blank"><?php /* phpcs:ignoreFile */ fs_esc_html_echo_inline( 'License issues?', 'license-issues', $slug ) ?></a>
            <?php /* phpcs:ignoreFile */ endif ?>

            <?php /* phpcs:ignoreFile */ $fs->do_action( 'connect/after_actions', $activation_state ) ?>
		</div><?php /* phpcs:ignoreFile */
			...
			if ( ! empty( $permissions ) ) : ?>
				<div class="fs-permissions">
                    <?php /* phpcs:ignoreFile */ if ( $require_license_key ) : ?>
                        <a class="fs-trigger wp-core-ui" href="#" tabindex="1" style="color: inherit;"><?php /* phpcs:ignoreFile */ echo sprintf(
                    ...
                    <?php /* phpcs:ignoreFile */ else : ?>
                        <a class="fs-trigger wp-core-ui" href="#" tabindex="1" style="color: inherit;"><?php /* phpcs:ignoreFile */ printf(
                                fs_esc_html_inline( 'This will allow %s to', 'this-will-allow-x', $slug ) . '<b class="fs-arrow"></b>',
                                sprintf( '<nobr class="button-link" style="color: inherit;">%s</nobr>', esc_html( $fs->get_plugin_title() ) )
                            ) ?></a>
                    <?php /* phpcs:ignoreFile */ endif ?>
					<ul><?php /* phpcs:ignoreFile */
			...
			<?php /* phpcs:ignoreFile */ endif ?>
		<?php /* phpcs:ignoreFile */ if ( $is_premium_code && $is_freemium ) : ?>
			<div class="fs-freemium-licensing">
				<p>
					<?php /* phpcs:ignoreFile */ if ( $require_license_key ) : ?>
						<?php /* phpcs:ignoreFile */ fs_esc_html_echo_inline( 'Don\'t have a license key?', 'dont-have-license-key', $slug ) ?>
						<a data-require-license="false" tabindex="1"><?php /* phpcs:ignoreFile */ fs_esc_html_echo_inline( 'Activate Free Version', 'activate-free-version', $slug ) ?></a>
					<?php /* phpcs:ignoreFile */ else : ?>
						<?php /* phpcs:ignoreFile */ fs_echo_inline( 'Have a license key?', 'have-license-key', $slug ) ?>
						<a data-require-license="true" tabindex="1"><?php /* phpcs:ignoreFile */ fs_esc_html_echo_inline( 'Activate License', 'activate-license', $slug ) ?></a>
					<?php /* phpcs:ignoreFile */ endif ?>
				</p>
			</div>
		<?php /* phpcs:ignoreFile */ endif ?>
		...
		<div class="fs-terms">
            <a class="fs-tooltip-trigger<?php /* phpcs:ignoreFile */ echo is_rtl() ? ' rtl' : '' ?>" href="<?php /* phpcs:ignoreFile */ echo esc_url( $freemius_activation_terms_url ) ?>" target="_blank" rel="noopener" tabindex="1">Powered by Freemius<?php /* phpcs:ignoreFile */ if ( $require_license_key ) : ?> <span class="fs-tooltip" style="width: 170px"><?php /* phpcs:ignoreFile */ echo esc_html( $fs->get_text_inline( 'Freemius is our licensing and software updates engine', 'permissions-extensions_desc' ) ) ?></span><?php /* phpcs:ignoreFile */ endif ?></a>
            &nbsp;&nbsp;-&nbsp;&nbsp;
			<a href="https://freemius.com/privacy/" target="_blank" rel="noopener"
			   tabindex="1"><?php /* phpcs:ignoreFile */ fs_esc_html_echo_inline( 'Privacy Policy', 'privacy-policy', $slug ) ?></a>
			&nbsp;&nbsp;-&nbsp;&nbsp;
			<?php /* phpcs:ignoreFile */ if ($require_license_key) : ?>
				<a href="<?php /* phpcs:ignoreFile */ echo esc_url( $freemius_plugin_terms_url ) ?>" target="_blank" rel="noopener" tabindex="1"><?php /* phpcs:ignoreFile */ fs_echo_inline( 'License Agreement', 'license-agreement', $slug ) ?></a>
			<?php /* phpcs:ignoreFile */ else : ?>
				<a href="<?php /* phpcs:ignoreFile */ echo esc_url( $freemius_usage_tracking_url ) ?>" target="_blank" rel="noopener" tabindex="1"><?php /* phpcs:ignoreFile */ fs_echo_inline( 'Terms of Service', 'tos', $slug ) ?></a>
			<?php /* phpcs:ignoreFile */ endif; ?>
		</div>
	</div>
	<?php /* phpcs:ignoreFile */
...
<script type="text/javascript">
	(function ($) {
		...
		$themeConnectWrapper.find('button.close').on('click', function () {
			<?php /* phpcs:ignoreFile */ if ( ! empty( $previous_theme_activation_url ) ) { ?>
			location.href = '<?php /* phpcs:ignoreFile */ echo html_entity_decode( $previous_theme_activation_url ); ?>';
			<?php /* phpcs:ignoreFile */ } else { ?>
		...
		var $primaryCta          = $('.fs-actions .button.button-primary'),
		    ...
		    isNetworkActive      = <?php /* phpcs:ignoreFile */ echo $is_network_level_activation ? 'true' : 'false' ?>,
		    requireLicenseKey    = <?php /* phpcs:ignoreFile */ echo $require_license_key ? 'true' : 'false' ?>,
		    hasContextUser       = <?php /* phpcs:ignoreFile */ echo $activate_with_current_user ? 'true' : 'false' ?>,
		    isNetworkUpgradeMode = <?php /* phpcs:ignoreFile */ echo $is_network_upgrade_mode ? 'true' : 'false' ?>,
		    $licenseSecret,
		...
		if ( isNetworkActive ) {
			var
				...
				totalSites                  = <?php /* phpcs:ignoreFile */ echo count( $sites ) ?>,
				maxSitesListHeight          = null,
				$skipActivationButton       = $( '#skip_activation' ),
				$delegateToSiteAdminsButton = $( '#delegate_to_site_admins' ),
                hasAnyInstall               = <?php /* phpcs:ignoreFile */ echo ! is_null( $fs->find_first_install() ) ? 'true' : 'false' ?>;
            ...
            if ( isNetworkUpgradeMode || hasAnyInstall ) {
                $skipActivationButton.click(function(){
                    ...
                    $skipActivationButton.html('<?php /* phpcs:ignoreFile */ fs_esc_js_echo_inline( 'Skipping, please wait', 'skipping-wait', $slug ) ?>...');
                ...
                $delegateToSiteAdminsButton.click(function(){
                    $delegateToSiteAdminsButton.html('<?php /* phpcs:ignoreFile */ fs_esc_js_echo_inline( 'Delegating, please wait', 'delegating-wait', $slug ) ?>...');
		...
		function updatePrimaryCtaText( actionType ) {
			...
			var text = '<?php /* phpcs:ignoreFile */ fs_esc_js_echo_inline( 'Continue', 'continue', $slug ) ?>';

			switch ( actionType ) {
				case 'allow':
					text = '<?php /* phpcs:ignoreFile */ fs_esc_js_echo_inline( 'Allow & Continue', 'opt-in-connect', $slug ) ?>';
					break;
				case 'delegate':
					text = '<?php /* phpcs:ignoreFile */ fs_esc_js_echo_inline( 'Delegate to Site Admins & Continue', 'delegate-to-site-admins-and-continue', $slug ) ?>';
					break;
				case 'skip':
					text = '<?php /* phpcs:ignoreFile */ fs_esc_js_echo_x_inline( 'Skip', 'verb', 'skip', $slug ) ?>';
					break;
			}
			...
			if ( ajaxOptin ) {
				if (!hasContextUser || isNetworkUpgradeMode) {
				    ...
				    if ( requireLicenseKey && ! isNetworkDelegating ) {
                        action   = '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_action( 'activate_license' ) ?>';
                        security = '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_security( 'activate_license' ) ?>';
                    } else {
                        action   = '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_action( 'network_activate' ) ?>';
                        security = '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_security( 'network_activate' ) ?>';
                    }
		...
		$primaryCta.on('click', function () {
			...
			$(this).html('<?php /* phpcs:ignoreFile */ echo esc_js( $is_pending_activation ?
		...
		if (requireLicenseKey) {
			    ...
			    var
                    ...
                    fetchIsMarketingAllowedFlagAndToggleOptin = function () {
                        ...
                        $primaryCta.attr('disabled', 'disabled');
                        $primaryCta.html('<?php /* phpcs:ignoreFile */ fs_esc_js_echo_inline( 'Please wait', 'please-wait', $slug ) ?>...');

                        $.ajax({
                            url    : <?php /* phpcs:ignoreFile */ echo Freemius::ajax_url() ?>,
                            method : 'POST',
                            data   : {
                                action     : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_action( 'fetch_is_marketing_required_flag_value' ) ?>',
                                security   : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_security( 'fetch_is_marketing_required_flag_value' ) ?>',
                                license_key: licenseKey,
                                module_id  : '<?php /* phpcs:ignoreFile */ echo $fs->get_id() ?>'
                            },
                            success: function (result) {
