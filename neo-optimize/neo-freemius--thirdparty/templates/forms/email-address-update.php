...
( function ( $ ) {
	var modalHtml                             =
		    ...
		    + '		    <h4><?php /* phpcs:ignoreFile */ fs_esc_attr_echo_inline( 'Email address update', 'email-address-update', $slug ) ?></h4>'
            + '         <a href="!#" class="fs-close"><i class="dashicons dashicons-no" title="<?php /* phpcs:ignoreFile */ echo esc_js( fs_text_x_inline( 'Dismiss', 'close window', 'dismiss', $slug ) ) ?>"></i></a>'
		    + '		</div>'
		    + '		<div class="fs-modal-body">'
		    + '			<div class="fs-modal-panel active">'
            + '             <div class="notice notice-error inline fs-update-error-message"><p></p></div>'
            + '             <p><?php /* phpcs:ignoreFile */ fs_echo_inline( 'Enter the new email address', 'enter-new-email-address' , $slug ) ?>:</p>'
            + '	    	    <input type="text" class="fs-new-email-address-input" />'
            + '             <div class="fs-email-addresses-ownership-options" style="display: none">'
            + '                 <p><?php /* phpcs:ignoreFile */ echo sprintf(
            ...
            + '                             <span><?php /* phpcs:ignoreFile */ fs_echo_inline( 'Yes - both addresses are mine', 'both-addresses-are-mine', $slug ); ?></span>'
            + '                         </label>'
            + '                     </li>'
            + '                     <li>'
            + '                         <label>'
            + '                             <span><input type="radio" name="email-addresses-ownership" value="current"/></span>'
            + '                             <div><?php /* phpcs:ignoreFile */ echo sprintf(
            ...
            + '                             <span><?php /* phpcs:ignoreFile */ echo sprintf(
		    ...
		    + '			<button class="button button-primary button-update" disabled><?php /* phpcs:ignoreFile */ fs_esc_js_echo_inline( 'Update', 'update-email-address', $slug ) ?></button>'
		    + '			<button class="button button-secondary button-close"><?php /* phpcs:ignoreFile */ fs_esc_js_echo_inline( 'Cancel', 'cancel', $slug ) ?></button>'
		    + '		</div>'
		    + '	</div>'
		    + '</div>',
	    $modal                                = $( modalHtml ),
	...
	function registerEventHandlers() {
		...
		$modal.on( 'click', '.fs-modal-footer .button-update', function ( evt ) {
            ...
            if ( previousEmailAddress === '<?php /* phpcs:ignoreFile */ echo $current_email_address ?>' ) {
                closeModal();
                return;
            }
            ...
            $.ajax( {
                ...
                method    : 'POST',
                data      : {
                    action       : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_action( 'update_email_address' ) ?>',
                    security     : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_security( 'update_email_address' ) ?>',
                ...
                error     : function () {
                    showError( '<?php /* phpcs:ignoreFile */ fs_esc_js_echo_inline( 'Unexpected error, try again in 5 minutes. If the error persists, please contact support.', 'unexpected-error', $slug ) ?>' );
	...
	function resetModal() {
        ...
        $updateButton.text( <?php /* phpcs:ignoreFile */ echo json_encode( fs_text_inline( 'Update', 'update-email-address', $slug ) ) ?> );
	}
