			...
			(function ($) {
				$(function () {
					...
					var
						...
						frame_height = 800,
						base_url     = '<?php /* phpcs:ignoreFile */ echo FS_CHECKOUT__ADDRESS ?>',
						// Pass the parent page URL into the i-frame in a meaningful way (this URL could be
						// passed via query string or hard coded into the child page, it depends on your needs).
						src          = base_url + '/?<?php /* phpcs:ignoreFile */ echo http_build_query( $query_params ) ?>#' + encodeURIComponent(document.location.href),
					...
					FS.PostMessage.receiveOnce('get_context', function () {
						...
						<?php /* phpcs:ignoreFile */ $install_data = array_merge( $fs->get_opt_in_params(),
						array(
						...
						FS.PostMessage.post('context', <?php /* phpcs:ignoreFile */ echo json_encode( $install_data ) ?>, frame[0]);
					});

					FS.PostMessage.receiveOnce('purchaseCompleted', <?php /* phpcs:ignoreFile */ echo $fs->apply_filters('checkout/purchaseCompleted', 'function (data) {
