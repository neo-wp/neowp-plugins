    ...
    class FS_Plugin_Neo_Updater {
        ...
        private function filters() {
            ...
            if ( $this->_fs->is_premium() ) {
                ...
                add_filter( 'upgrader_pre_install', array( 'FS_Plugin_Neo_Updater', '_store_basename_for_source_adjustment' ), 1, 2 );
                add_filter( 'upgrader_source_selection', array( 'FS_Plugin_Neo_Updater', '_maybe_adjust_source_dir' ), 1, 3 );
                ...
                if ( typeof jQuery !== 'undefined' ) {
                    jQuery( document ).on( 'wp-plugin-updating', function( event, args ) {
                        if ( typeof args === 'object' && args.slug && typeof args.slug === 'string' ) {
                            if ( <?php /* phpcs:ignoreFile */ echo json_encode( $this->_fs->get_slug() ) ?> === args.slug ) {
        ...
        private function add_transient_filters() {
            ...
            add_filter( 'pre_set_site_transient' . '_update_plugins', array(
                &$this,
                'pre_set_site_transient_neo_update_plugins_filter'
            ) );

            add_filter( 'pre_set_site_transient' . '_update_themes', array(
                &$this,
                'pre_set_site_transient_neo_update_plugins_filter'
            ) );
        }
        ...
        private function remove_transient_filters() {
            remove_filter( 'pre_set_site_transient' . '_update_plugins', array(
                &$this,
                'pre_set_site_transient_neo_update_plugins_filter'
            ) );

            remove_filter( 'pre_set_site_transient' . '_update_themes', array(
                &$this,
                'pre_set_site_transient_neo_update_plugins_filter'
            ) );
        }
         ...
         */
        function pre_set_site_transient_neo_update_plugins_filter( $transient_data ) {
...
if ( !isset($info->error) ) {
    $data = $info;
        ...
        private function get_latest_download_details( $addon_id = false, $newer_than = false, $fetch_readme = true ) {
            return $this->_fs->_fetch_latest_version( $addon_id, true, FS_Plugin_Neo_Updater::UPDATES_CHECK_CACHE_EXPIRATION, $newer_than, $fetch_readme );
        }
...
//			$skin = new Plugin_Installer_Skin( $skin_args );
            ...
            remove_filter( 'upgrader_source_selection', array( 'FS_Plugin_Neo_Updater', '_maybe_adjust_source_dir' ), 1 );
