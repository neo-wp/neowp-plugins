...
function replace_media_urls_in_db($urls_to_replace, $skip_thread_stack_retry = false) {
    global $wpdb;
    $urls_to_replace = \NeoReplace\NeoGlobal\array_filter_better($urls_to_replace, fn ($url_to_replace) => ($url_to_replace["old_url"] ?? "") !== "" && ($url_to_replace["new_url"] ?? "") !== "" && $url_to_replace["old_url"] !== $url_to_replace["new_url"]);
    ...
    foreach ($urls_to_replace as $url_to_replace) {
        ...
        $old_image_url_relative = \NeoReplace\NeoGlobal\make_internal_url_relative($old_image_url_absolute);
        $new_image_url_relative = \NeoReplace\NeoGlobal\make_internal_url_relative($new_image_url_absolute);
        ...
        $old_path_rel_with_dir = \NeoReplace\NeoGlobal\make_internal_url_relative_to_uploads($old_image_url_absolute);
        $new_path_rel_with_dir = \NeoReplace\NeoGlobal\make_internal_url_relative_to_uploads($new_image_url_absolute);
        ...
        $lower_percent_encoding = function ($url_encoded_string) { return \NeoReplace\NeoGlobal\preg_replace_callback_better("/%[0-9A-F]{2}/", fn ($match) => strtolower($match[0]), $url_encoded_string); };
    ...
    $replacements1 = \NeoReplace\NeoGlobal\array_unique_better($replacements1);
    $replacements2 = \NeoReplace\NeoGlobal\array_unique_better($replacements2);
    $replacements3 = \NeoReplace\NeoGlobal\array_unique_better($replacements3);

    $replacing_file_extensions = \NeoReplace\NeoGlobal\array_filter_better(\NeoReplace\NeoGlobal\array_unique_better(array_map(function ($url) { return strtolower(pathinfo(wp_parse_url($url["old_url"], PHP_URL_PATH) ?: $url["old_url"], PATHINFO_EXTENSION)); }, $urls_to_replace)));
    ...
    $old_replacement_strings_for_lcs = \NeoReplace\NeoGlobal\array_unique_better(\NeoReplace\NeoGlobal\array_filter_better(array_map(fn ($replacement) => $replacement["old_str"] ?? "", array_merge($replacements1, $replacements2, $replacements3))));
    ...
    foreach ($sql_scripts as $sql_script) {
        for ($lock_attempt = 1; $lock_attempt <= 10; $lock_attempt++) {
            ...
            $error_string = \NeoReplace\NeoGlobal\neo__("Error while locking database rows. Database error: ", "Fehler beim Sperren der Datenbank-Zeilen. Datenbankfehler: ") . ($wpdb->last_error ?: "-") . \NeoReplace\NeoGlobal\neo__(" SQL command: ", " SQL Befehl: ") . $sql_script["lock_candidates_sql_cmd"]; break;
        }
    ...
    foreach ($sql_scripts as $sql_script) {
        ...
        for ($update_attempt = 1; $update_attempt <= 10; $update_attempt++) {
            ...
            $error_string = \NeoReplace\NeoGlobal\neo__("Error while renaming URLs in database. Database error: ", "Fehler beim Umbenennen von URLs in der Datenbank. Datenbankfehler: ") . ($wpdb->last_error ?: "-") . \NeoReplace\NeoGlobal\neo__(" SQL command: ", " SQL Befehl: ") . $sql_script["sql_cmd"]; break;
        }
    }
    ...
    if ($error_string !== null && $db_error_code === 1436 && $thread_stack_savepoint_created) {
        ...
        for ($thread_stack_batch_count = 2; $thread_stack_batch_count <= count($urls_to_replace); $thread_stack_batch_count = min(count($urls_to_replace), $thread_stack_batch_count * 2)) {
            if ($wpdb->query("ROLLBACK TO SAVEPOINT $thread_stack_savepoint_name") === false) { return $original_error_string . \NeoReplace\NeoGlobal\neo__(" Retrying failed because the database savepoint could not be restored: ", " Erneuter Versuch fehlgeschlagen, weil der Datenbank-Savepoint nicht wiederhergestellt werden konnte: ") . ($wpdb->last_error ?: "-"); }
            $error_string = null;
            \NeoReplace\NeoGlobal\global_log_with_module_name("_global-rename-image-engine", "Retry media URL replacements after database thread stack overrun: " . \NeoReplace\NeoGlobal\json_encode_better(["urlCount" => count($urls_to_replace), "batchCount" => $thread_stack_batch_count]));
