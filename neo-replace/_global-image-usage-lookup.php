...
function db_entries_image_usage_lookup($img_urls, $max_sql_time_seconds = 20, $include_content_preview = true) {
    ...
    $get_results_with_timeout = function ($query, $output = OBJECT) use ($wpdb, $max_sql_time_seconds, $query_start_time, $db_is_mariadb) {
        ...
        $timed_query = $db_is_mariadb ? "SET STATEMENT max_statement_time=" . sprintf("%.3F", $remaining_time_seconds) . " FOR " . $query : \NeoReplace\NeoGlobal\preg_replace_better("/^SELECT\\h+/i", "SELECT /*+ MAX_EXECUTION_TIME(" . max(1, intval(ceil($remaining_time_seconds * 1000))) . ") */ ", $query);
    ...
    $img_url_file_extensions = \NeoReplace\NeoGlobal\array_unique_better($img_url_file_extensions);
    ...
    $normalize_img_usage_lookup_key = function ($key) { return strtolower(\NeoReplace\NeoGlobal\preg_replace_better("/[^a-z0-9]+/i", "", (string) $key)); };
    ...
    $img_usage_lookup_value_has_contextual_img_id = function ($value, $img_id_as_string, $row, $column_name) use ($img_id_media_context_terms, $img_id_layout_context_terms, $img_usage_lookup_key_path_contains_term, $img_usage_lookup_row_context_key_path) {
        ...
        if (!\NeoReplace\NeoGlobal\preg_match_better("/(^|[^0-9])" . preg_quote($img_id_as_string, "/") . "([^0-9]|$)/", $value_as_string)) { return false; }
        ...
        if ($trimmed_value !== "" && in_array($trimmed_value[0], ["{", "["], true) && ($decoded_json_value = \NeoReplace\NeoGlobal\json_decode_better($trimmed_value, suppress_error: true)) !== false && $find_contextual_img_id($decoded_json_value, $row_context_key_path)) { return true; }
        if (!$is_blocked_key_path($row_context_key_path) && $is_media_key_path($row_context_key_path) && \NeoReplace\NeoGlobal\preg_match_better("/(^|,)\\h*\"?" . preg_quote($img_id_as_string, "/") . "\"?\\h*(,|$)/", $value_as_string)) { return true; }
        $unstructured_key_value_matches = []; \NeoReplace\NeoGlobal\preg_match_all_better("/(?<key>[a-zA-Z0-9_.-]{2,})\\h*(?::|=>|=)\\h*\\\\?\"?" . preg_quote($img_id_as_string, "/") . "\\\\?\"?(?=[^0-9]|$)/i", $value_as_string, $unstructured_key_value_matches, PREG_SET_ORDER);
            ...
            foreach ($rows_containing_img_ids as $row) {
                foreach ($img_id_as_string_by_img_url as $img_url => $img_id_as_string) {
                    ...
                    foreach ($row as $col_name_for_preview => $value) {
                        ...
                        $exact_img_id_preview_matches = []; $img_id_pos = $includes_img_id && \NeoReplace\NeoGlobal\preg_match_better("/(^|[^0-9])" . preg_quote($img_id_as_string, "/") . "([^0-9]|$)/", (string) $value, $exact_img_id_preview_matches, PREG_OFFSET_CAPTURE) ? $exact_img_id_preview_matches[0][1] + strlen($exact_img_id_preview_matches[1][0]) : 0;
    ...
    foreach ($db_entries_using_images as $img_url => &$db_entries_using_img) {
        ...
        $db_entries_using_img = \NeoReplace\NeoGlobal\array_filter_better($db_entries_using_img, function ($db_entry) use (&$already_seen_db_entry_keys, $wpdb) {
            ...
            else { $db_entry_key = $db_entry["tableName"] . "|" . \NeoReplace\NeoGlobal\json_encode_better($content_preview); }
    ...
    foreach ($db_entries_using_images as $img_url => &$db_entries_using_img) {
        foreach ($db_entries_using_img as &$db_entry) {
            ...
            if ($row_preview_post instanceof \WP_Post) {
                ...
                $db_entry["postUrlDisplayPath"] = \NeoReplace\NeoGlobal\make_internal_url_relative($db_entry["postUrl"]);
                $db_entry["postUrlDisplayPath"] = \NeoReplace\NeoGlobal\preg_replace_better("#^/scope:[^/]+(?=/)#", "", $db_entry["postUrlDisplayPath"]);
    ...
    foreach ($db_entries_using_images as $img_url => &$db_entries_using_img) {
        $db_entries_using_img = \NeoReplace\NeoGlobal\array_filter_better($db_entries_using_img, function ($db_entry) use ($option_names_skipped_for_usage_lookup) {
...
\NeoReplace\NeoGlobal\register_rest_endpoint("/wp-json/neo/db-entries-usage-neo-replace", "GET", fn () => \NeoReplace\NeoGlobal\current_user_can__global_db_entries_usage(), function ($get_param) {
    $img_url = \NeoReplace\NeoGlobal\percent_decode_invalid_utf8_url_bytes(\NeoReplace\NeoGlobal\remove_all_query_params((string) $get_param("img-url")));
    if (!$img_url) { \NeoReplace\NeoGlobal\throw_global_exception("Missing image URL.", status_code: 400); }
    $timeout_seconds = max(0, (int) ($get_param("timeout") ?: 10));

    $usage_lookup = db_entries_image_usage_lookup([$img_url], max_sql_time_seconds: $timeout_seconds, include_content_preview: \NeoReplace\NeoGlobal\current_user_can__global_db_entries_details());
    return ["dbEntriesUsingImage" => $usage_lookup === false ? false : ($usage_lookup[$img_url] ?? []), "timeoutSeconds" => $timeout_seconds, "imgUrl" => \NeoReplace\NeoGlobal\percent_encode_invalid_utf8_url_bytes($img_url)];
});

\NeoReplace\NeoGlobal\add_action_hook("neo_init", function () {
    $db_entries_usage_url = \NeoReplace\NeoGlobal\get_backend_page_url("neo-global--db-entries-usage-neo-replace");
    \NeoReplace\NeoGlobal\enqueue_js_variable_backend("neoGlobalDbEntriesUsageUrl", $db_entries_usage_url);
});
