...
function wp_date_string($format = "Y-m-d H:i:s", $timestamp = null) {
    if (!is_string($format)) { \NeoReplace\NeoGlobal\throw_global_exception("Invalid format (not a string)"); }
    if (is_float($timestamp)) { $timestamp = intval($timestamp); }
    if ($timestamp !== null && !is_int($timestamp)) { \NeoReplace\NeoGlobal\throw_global_exception("Invalid timestamp: " . $timestamp); }
...
function utc_date_string($format = "Y-m-d H:i:s", $timestamp = null) {
    if (!is_string($format)) { \NeoReplace\NeoGlobal\throw_global_exception("Invalid format (not a string): " . $format); }
    if (is_float($timestamp)) { $timestamp = intval($timestamp); }
    if ($timestamp !== null && !is_int($timestamp)) { \NeoReplace\NeoGlobal\throw_global_exception("Invalid timestamp: " . $timestamp); }
...
function timestamp_from_wp_date_string($date_string, $format = "Y-m-d H:i:s") {
    ...
    if ($date_time === false) { \NeoReplace\NeoGlobal\throw_global_exception("Could not convert date string to timestamp. Date string: " . $date_string . "."); }
    return $date_time->getTimestamp();
}

function timestamp_from_utc_date_string($date_string, $format = "Y-m-d H:i:s") {
    if (!is_string($date_string)) { \NeoReplace\NeoGlobal\throw_global_exception("Invalid date string (not a string): " . \NeoReplace\NeoGlobal\json_encode_better($date_string)); }
    if (!is_string($format))      { \NeoReplace\NeoGlobal\throw_global_exception("Invalid format (not a string): " .      \NeoReplace\NeoGlobal\json_encode_better($format)); }
