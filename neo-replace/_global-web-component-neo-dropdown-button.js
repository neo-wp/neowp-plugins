...
export default class NeoDropdownButton extends HTMLElement {
    ...
    constructor() {
        ...
        this.shadowRoot.innerHTML = `
            <style>
                ...
                neo-select-neo-replace, button { font-size: 1em; }
                neo-select-neo-replace         { --neo-select-border-color-neo-replace: #C0C0C0; --neo-select-border-radius-neo-replace: 10px 0 0 10px; --neo-select-padding-neo-replace: 0.8em 1.9em 0.8em 0.8em; }
                button                                      { border: 1px solid #C0C0C0; }
                button                                      { border-radius: 0 10px 10px 0; padding: 0.8em 1.0em 0.8em 1.0em; }
                :host([only-round-corner-top-left]) neo-select-neo-replace { --neo-select-border-radius-neo-replace: 10px 0 0 0; }
                :host([only-round-corner-top-left]) button  { border-radius: 0; }
                button                                      { margin-left: -1px; }
                neo-select-neo-replace         { max-width: calc(100vw - 140px); }
            ...
            <neo-select-neo-replace></neo-select-neo-replace>
            <slot style="display:none;"></slot>
            <button><span class="button-text">OK</span><span class="button-text-success">${neo__("Done", "Fertig")}</span></button>
        `;
        this._select = this.shadowRoot.querySelector("neo-select-neo-replace");
    ...
    connectedCallback() {
        (async () => {
            await customElements.whenDefined("neo-select-neo-replace");
    ...
    get value()           { return this._select?.value ?? this._pendingValue ?? ""; } set value(newValue) { this._pendingValue = newValue ?? ""; if (customElements.get("neo-select-neo-replace")) { this._applyValue(this._pendingValue); } }
