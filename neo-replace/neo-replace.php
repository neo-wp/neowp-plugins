<?php
/*
* Plugin Name: neo Replace
* Plugin URI: https://neo-wp.com/?plugin-slug=neo-replace&edition=wporg&ref=entry-file
* Description: Simple replacement with drag and drop!
* Version: 26.8.31
* Requires PHP: 8.0
...
* Edition: wporg
* Slug: neo-replace
* Filename: neo-replace.26.8.31.zip
* Date: 2026-08-31 17:15:38 (Europe/Berlin)
...
namespace NeoReplace\NeoGlobal {
...
namespace NeoReplace\NeoEntrypoint {
    // Abort if this plugin is loaded twice with different names.
    if (!function_exists("\\" . __NAMESPACE__ . "\\" . "dummy_function")) { function dummy_function() { }
        ...
        function suppress_plugin_on_error($callback) {
            try { return $callback(); }
                ...
                if (\NeoReplace\NeoGlobal\str_contains($error_message, "#ignoreNeoSuppress")) { throw $e; }
                if (defined("WP_CLI") && WP_CLI) { throw $e; }
                try {
                    if (!($e instanceof \NeoReplace\NeoGlobal\GlobalException)) {
                        if (function_exists('\NeoReplace\NeoGlobal\global_warn')) { \NeoReplace\NeoGlobal\global_warn_with_module_name("neowp", "PHP Error in plugin: $error_message"); }
                    }
                } catch (\Throwable $inner_e) {}
                suppress_plugin($error_message);

                if (file_exists(__DIR__ . "/_global--error-message-page-only-debug.php")) { (fn () => require_once(__DIR__ . "/_global--error-message-page-only-debug.php"))(); \NeoReplace\NeoGlobal\show_better_error_message_page($e); }
        ...
        suppress_plugin_on_error(function () {
            ...
            if (\NeoReplace\NeoGlobal\str_contains($query_string, "neosuppress")) {
                suppress_plugin("Plugin is suppressed by ?neosuppress in the URL.");
            }
            if (\NeoReplace\NeoGlobal\str_contains($query_string, "neoundosuppress")) {
            ...
            if (!empty($neo_suppress_cookie)) {
                ...
                if (\NeoReplace\NeoGlobal\str_contains($query_string, "page=" . \NeoReplace\NeoGlobal\plugin_settings_page_slug())) { add_action("init", function () { wp_safe_redirect(admin_url()); exit; }); }
            ...
            if (in_array(PHP_OS_FAMILY, \NeoReplace\NeoGlobal\plugin_blocked_os_families()))                { $neo_load_error_message = "neoPlugin is inactive because it is not supported on " . PHP_OS_FAMILY . "."; }
            if (!version_compare(PHP_VERSION, \NeoReplace\NeoGlobal\plugin_min_php_ver(), ">="))            { $neo_load_error_message = "neoPlugin is inactive because PHP is too old (" . PHP_VERSION . " < " . \NeoReplace\NeoGlobal\plugin_min_php_ver() . ")."; }
            if (!version_compare(get_bloginfo("version"), \NeoReplace\NeoGlobal\plugin_min_wp_ver(), ">=")) { $neo_load_error_message = "neoPlugin is inactive because WP is too old (" . get_bloginfo("version") . " < " . \NeoReplace\NeoGlobal\plugin_min_wp_ver() . ")."; }
            ...
            function _neo_entry_function_get_module_name($rel_path) {
                if (\NeoReplace\NeoGlobal\str_starts_with($rel_path, "_global-")) { return "neo-global"; }
            ...
            foreach (scandir(dirname(__FILE__)) as $plugin_file_name) {
                ...
                if (!\NeoReplace\NeoGlobal\str_ends_with($plugin_file_name, ".php"))                { continue; }
                if ($plugin_file_name === "index.php")                                   { continue; }
                if ($plugin_file_name === basename(\NeoReplace\NeoGlobal\plugin_entry_file_path())) { continue; }
                if (\NeoReplace\NeoGlobal\str_starts_with($plugin_file_name, "_global-")) { $global_files[] = $plugin_file_name; }
            ...
            \NeoCore\NeoSuperGlobal::$globals["active_plugins"][] = [
                "slug" => \NeoReplace\NeoGlobal\plugin_slug(), "edition" => \NeoReplace\NeoGlobal\plugin_edition(),
                "version" => \NeoReplace\NeoGlobal\plugin_version(),
                "namespace_prefix" => \NeoReplace\NeoGlobal\plugin_namespace_prefix(),
                "plugin_path" => dirname(__FILE__),
                "plugin_entry_file_path" => \NeoReplace\NeoGlobal\plugin_entry_file_path(),
            ...
            function master_function($is_special_call_for_activation_or_uninstallation = false) {
                suppress_plugin_on_error(function () use ($is_special_call_for_activation_or_uninstallation) {
                    ...
                    \NeoReplace\NeoGlobal\performance_checkpoint("live update", time: $live_update_time_before, is_start: true);
                    \NeoReplace\NeoGlobal\performance_checkpoint("live update", time: $live_update_time_after,  is_end: true);
                    \NeoReplace\NeoGlobal\performance_checkpoint("plugin-load", is_start: true);
                    ...
                    foreach (\NeoCore\NeoSuperGlobal::$globals["active_plugins"] as $plugin) {
                        foreach ($plugin["global_files"] as $global_file) {
                            ...
                            \NeoReplace\NeoGlobal\performance_checkpoint("load $global_file", is_start: true);
                            (fn () => require_once($plugin["plugin_path"] . "/" . $global_file))();
                            \NeoReplace\NeoGlobal\performance_checkpoint("load $global_file", is_end: true);
                        }
                    }

                    if (\NeoReplace\NeoGlobal\array_filter_better(\NeoCore\NeoSuperGlobal::$globals["active_plugins"], fn ($active_plugin) => array_key_exists("neo-redirect", $active_plugin["module_files"]))) {
                    ...
                    if (\NeoReplace\NeoGlobal\array_filter_better(\NeoCore\NeoSuperGlobal::$globals["active_plugins"], fn ($active_plugin) => array_key_exists("neo-manager-installer", $active_plugin["module_files"]))) {
                        foreach (\NeoCore\NeoSuperGlobal::$globals["active_plugins"] as &$active_plugin_ref) {
                            foreach ($active_plugin_ref["module_files"] as $module_slug => $module_files) {
                                foreach (\NeoReplace\NeoGlobal\option__neo_manager_installer__suppressed_plugins_list() as $suppressed_plugin_slug) {
                                    if (\NeoReplace\NeoGlobal\str_starts_with($module_slug, $suppressed_plugin_slug)) {
                    ...
                    foreach (\NeoCore\NeoSuperGlobal::$globals["active_plugins"] as $active_plugin) {
                        foreach ($active_plugin["module_files"] as $module_slug => $module_files) {
                            ...
                            foreach ($module_files as $module_file) {
                                ...
                                \NeoReplace\NeoGlobal\performance_checkpoint("load $module_file", is_start: true);
                                (fn () => require_once($active_plugin["plugin_path"] . "/" . $module_file))();
                                \NeoReplace\NeoGlobal\performance_checkpoint("load $module_file", is_end: true);
                            }
                        }
                    }
                    ...
                    \NeoReplace\NeoGlobal\performance_checkpoint("plugin-load", is_end: true);
                });
            }

            \NeoCore\NeoCoreMaster::register_master_function(\NeoReplace\NeoGlobal\plugin_timestamp(), __NAMESPACE__ . "\\master_function");
