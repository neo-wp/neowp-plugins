...
<script type="text/javascript">
    (function( $ ) {
        var modalHtml =
            '<div class="fs-modal fs-modal-subscription-cancellation fs-modal-subscription-cancellation-<?php /* phpcs:ignoreFile */ echo $fs->get_id() ?>">'
            + '	<div class="fs-modal-dialog">'
            + '		<div class="fs-modal-header">'
            + '		    <h4><?php /* phpcs:ignoreFile */ echo esc_html( sprintf( fs_text_inline( 'Cancel %s?', 'cancel-x', $slug ), ucfirst( $subscription_cancellation_context ) ) ) ?></h4>'
            + '		</div>'
            + '		<div class="fs-modal-body">'
            + '			<div class="fs-modal-panel active">' + <?php /* phpcs:ignoreFile */ echo json_encode( $subscription_cancellation_html ) ?> + '<p class="fs-price-increase-warning" style="display: none;">' + <?php /* phpcs:ignoreFile */ echo json_encode( $prices_increase_text ) ?> + '</p></div>'
            + '		</div>'
            + '		<div class="fs-modal-footer">'
            + '			<a href="#" class="button button-secondary button-close"><?php /* phpcs:ignoreFile */ fs_esc_attr_echo_inline( 'Cancel', 'cancel', $slug ) ?></a>'
            + '			<a href="#" class="button button-primary button-deactivate disabled"><?php /* phpcs:ignoreFile */ fs_esc_attr_echo_inline( 'Proceed', 'proceed', $slug ) ?></a>'
            + '		</div>'
            + '	</div>'
            + '</div>',
            $modal    = $(modalHtml);
        ...
        function registerEventHandlers() {
            ...
            $modal.on('click', '.fs-modal-footer .button', function (evt) {
                ...
                var _this                                   = $(this),
                    subscriptionCancellationActionEventName = <?php /* phpcs:ignoreFile */ echo json_encode( $fs->get_action_tag( 'subscription_cancellation_action' ) ) ?>;
                ...
                if ( _this.hasClass( 'button-primary' ) ) {
                    if ( 'true' !== $modal.find( 'input[name="cancel-subscription"]:checked' ).val() ) {
                        ...
                        if ( confirm( <?php /* phpcs:ignoreFile */ echo json_encode( $subscription_cancellation_confirmation_message ) ?> ) ) {
            ...
            $modal.on('click', 'input[type="radio"]', function () {
                ...
                if ( isSelected ) {
                    $primaryButton.html( <?php /* phpcs:ignoreFile */ echo json_encode( sprintf(
                    ...
                    $primaryButton.html( <?php /* phpcs:ignoreFile */ fs_json_encode_echo_inline( 'Proceed', 'proceed', $slug ) ?> );
        ...
        function updateButtonLabels() {
            $modal.find('.button-primary').text( <?php /* phpcs:ignoreFile */ fs_json_encode_echo_inline( 'Proceed', 'proceed', $slug ) ?> );

            $modal.find('.button-secondary').text( <?php /* phpcs:ignoreFile */ fs_json_encode_echo_inline( 'Cancel', 'cancel', $slug ) ?> );
        }
    })( jQuery );
</script>