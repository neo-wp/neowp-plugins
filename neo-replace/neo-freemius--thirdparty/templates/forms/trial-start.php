		...
		$(document).ready(function () {
			var modalContentHtml = <?php /* phpcs:ignoreFile */ echo json_encode( $modal_content_html ); ?>,
			    modalHtml        =
				    ...
				    + '		    <h4><?php /* phpcs:ignoreFile */ echo esc_js( fs_text_x_inline( 'Start free trial', 'call to action', 'start-free-trial', $slug ) ) ?></h4>'
				    + '		</div>'
				    + '		<div class="fs-modal-body">' + modalContentHtml + '</div>'
				    + '		<div class="fs-modal-footer">'
				    + '			<button class="button button-secondary button-close">' + <?php /* phpcs:ignoreFile */ fs_json_encode_echo_inline( 'Cancel', 'cancel', $slug ) ?> +'</button>'
				    + '			<button class="button button-primary button-connect">' + <?php /* phpcs:ignoreFile */ fs_json_encode_echo_inline( 'Approve & Start Trial', 'approve-start-trial', $slug ) ?> +'</button>'
				    + '		</div>'
				    + '	</div>'
				    + '</div>',
			    $modal = $( modalHtml ),
			...
			function registerEventHandlers() {
				...
				$modal.on('click', '.button-connect', function (evt) {
					...
					$.ajax({
						...
						method    : 'POST',
						data      : {
							action   : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_action( 'start_trial' ) ?>',
							security : '<?php /* phpcs:ignoreFile */ echo $fs->get_ajax_security( 'start_trial' ) ?>',
						...
						beforeSend: function () {
							...
							$button.text(<?php /* phpcs:ignoreFile */ fs_json_encode_echo_inline( 'Starting trial', 'starting-trial', $slug ) ?> + '...');

							setLoadingMode();
						},
						success   : function (resultObj) {
							if (resultObj.success) {
								$button.text(<?php /* phpcs:ignoreFile */ fs_json_encode_echo_inline( 'Please wait', 'please-wait', $slug ) ?> + '...');

								// Redirect to the "Account" page and sync the license.
								window.location.href = resultObj.data.next_page;
							} else {
								$button.text(<?php /* phpcs:ignoreFile */ fs_json_encode_echo( 'approve-start-trial', $slug ) ?>);
