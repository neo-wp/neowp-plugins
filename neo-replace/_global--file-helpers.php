...
function fs_file_get_contents($filename, $use_include_path = false, $context = null, $offset = 0, $length = null) { if ($context !== null) \NeoReplace\NeoGlobal\throw_global_exception("fs_file_get_contents() doesn't support the \$context parameter."); return ($length === null) ? file_get_contents($filename, $use_include_path, null, $offset) : file_get_contents($filename, $use_include_path, null, $offset, $length); }
...
function fs_read_json_file($file_path, $missing_fallback = [], $gzip = false) {
    ...
    return \NeoReplace\NeoGlobal\json_decode_better($file_contents);
}

function fs_write_json_file($file_path, $data, $gzip = false) {
    $json_content = \NeoReplace\NeoGlobal\json_encode_better($data, pretty_print: true);
    if ($gzip) { $json_content = gzencode($json_content); if ($json_content === false) { \NeoReplace\NeoGlobal\throw_global_exception("fs_write_json_file: Gzip encode failed for file path: $file_path"); } }
...
function fs_file_put_contents_better($file_path, $new_content) {
    ...
    $temp_file_path = \NeoReplace\NeoGlobal\cache_path("neo-global--file-helpers") . "/" . str_replace("/", "__", $file_path);
    try {
...
function sync_folder($src, $dest) {
    if (!fs_is_dir($src)) { \NeoReplace\NeoGlobal\throw_global_exception('Source "' . $src . '" is not a directory or does not exist'); }
    if (str_starts_with($dest, $src . "/") || $dest === $src) { \NeoReplace\NeoGlobal\throw_global_exception('Destination "' . $dest . '" is inside source "' . $src . '" or is the same as source'); }
    mkdir_better($dest);
    foreach (iterate_all_files($dest) as $file_path) { if (!fs_file_exists(\NeoReplace\NeoGlobal\str_replace_start($dest, $src, $file_path)) && !fs_fnmatch(".lock*", basename($file_path))) { delete_all($file_path); } }

    $temp_file_path = \NeoReplace\NeoGlobal\cache_path("neo-global--file-helpers") . "/sync-folder-temp-" . \NeoReplace\NeoGlobal\unique_planck_date();
    try {
        copy_all($src, $temp_file_path);

        foreach (iterate_all_files($temp_file_path, with_folders: true) as $file_path) {
            $dest_path = \NeoReplace\NeoGlobal\str_replace_start($temp_file_path, $dest, $file_path);
...
function copy_all($src, $dest, $copy_filter_file_callback = null) {
    ...
    foreach (new \FilesystemIterator($src, \FilesystemIterator::SKIP_DOTS) as $file) {
        ...
        if (!$success) { \NeoReplace\NeoGlobal\throw_global_exception('Could not copy file "' . $file->getPathname() . '" to "' . $dest . "/" . $file->getFilename() . '"'); }
    }
}
...
function delete_all($path, $filter = null) {
    ...
    if (!str_starts_with($path, $trash_dir_prefix) && !$filter) {
        $trash_path = $trash_dir_prefix . \NeoReplace\NeoGlobal\unique_planck_date() . "_" . basename($path);
...
function mkdir_better($path) {
    ...
    if (!str_starts_with($path, "/")) { \NeoReplace\NeoGlobal\throw_global_exception("Path must be absolute: $path"); }
    if (fs_is_dir($path)) { return; }
    if (fs_file_exists($path)) { \NeoReplace\NeoGlobal\throw_global_exception("Path exists and is not a directory: $path"); }
    ...
    for ($i = count($path_list) - 1; $i >= 0; $i--) {
        ...
        if (fs_file_exists($path_list[$i])) { \NeoReplace\NeoGlobal\throw_global_exception("Path exists and is not a directory: " . $path_list[$i]); }
    }
    ...
    for ($i = $start_index; $i < count($path_list); $i++) {
        ...
        if (!$success) { \NeoReplace\NeoGlobal\throw_global_exception("Could not create directory \"$current\". Please check the permissions of the parent folder: " . dirname($current)); }
        $parent = dirname($current);
        $base   = realpath($parent) ?: $parent;
        $perm   = fileperms($base) & 0777;
        $success = fs_chmod($current, $perm);
        if (!$success) { \NeoReplace\NeoGlobal\throw_global_exception("Could not set permissions on directory \"$current\""); }
    }
