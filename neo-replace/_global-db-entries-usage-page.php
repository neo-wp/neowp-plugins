...
\NeoReplace\NeoGlobal\register_backend_page("neo-global--db-entries-usage-neo-replace", fn () => \NeoReplace\NeoGlobal\current_user_can__global_db_entries_usage(), function () {
    \NeoReplace\NeoGlobal\enqueue_js_variable_backend("neoGlobalDbEntriesUsageDefaultTimeoutSeconds", 10);
    ?>
    <!DOCTYPE html>
    <html>
    <head>
        ...
        <title><?php \NeoReplace\NeoGlobal\echo_neo__("Image in WordPress Database", "Bilder in WordPress Datenbank") ?></title>
        <?php \NeoReplace\NeoGlobal\backend_page_style_tag_start([]); ?>
            ...
            .tableContainer>h2 {
                margin-bottom: 8px; color: <?php echo esc_html(\NeoReplace\NeoGlobal\transform_colors("#ff00ff")) ?>; font-size: 22px;
            }
            table {
            ...
            .highlight {
                background-color: <?php echo esc_html(\NeoReplace\NeoGlobal\transform_colors("#00ff00")) ?>;
            }
        ...
        <?php \NeoReplace\NeoGlobal\backend_page_script_tag_start([]); ?>
            <?php echo \NeoReplace\NeoGlobal\get_code_for_js_variables("backend") /* phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped */ ?>; /* Output is properly escaped in get_code_for_js_variables() for security */
        <?php \NeoReplace\NeoGlobal\backend_page_script_tag_end(); ?>
    </head>
    <body>
        <div id="pageHeader">
            <h1><?php \NeoReplace\NeoGlobal\echo_neo__("Image in WordPress Database", "Bilder in WordPress Datenbank") ?></h1>
            <p><?php \NeoReplace\NeoGlobal\echo_neo__("This page displays the database entries in WordPress that the image is using.", "Diese Seite zeigt die Datenbankeinträge in WordPress, die das Bild verwenden.") ?></p>
        </div>
        <div id="tablesContainer"></div>

        <?php \NeoReplace\NeoGlobal\backend_page_script_tag_start(["type" => "module"]); ?>
            import { neo__ } from "<?php echo esc_url(\NeoReplace\NeoGlobal\plugin_url()) ?>/_global--translation.js";
            import { jsVar } from "<?php echo esc_url(\NeoReplace\NeoGlobal\plugin_url()) ?>/_global--enqueue-loader.js";
            import { fetchImageUsageLookup } from "<?php echo esc_url(\NeoReplace\NeoGlobal\plugin_url()) ?>/_global-image-usage-lookup.js";
