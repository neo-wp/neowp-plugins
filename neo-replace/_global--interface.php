...
function call_interface_func_implemented($function_name_with_namespace, $throw_if_interface_not_ok = false) {
    ...
    if ($module_namespace === "NeoGlobal") {
        return function (...$args) use ($module_namespace, $function_name, $throw_if_interface_not_ok) {
            $active_plugins = \NeoReplace\NeoEntrypoint\get_neo_active_plugins();
            $this_plugin_from_active_plugins = \NeoReplace\NeoGlobal\array_filter_better($active_plugins, fn ($p) => ($p["slug"] === \NeoReplace\NeoGlobal\plugin_slug() && $p["edition"] === \NeoReplace\NeoGlobal\plugin_edition()))[0] ?? null;
            if ($this_plugin_from_active_plugins === null) { throw new \Exception(esc_html("Cannot find own plugin info in active plugins list.")); }
            $active_plugins = array_values(array_merge([$this_plugin_from_active_plugins], \NeoReplace\NeoGlobal\array_filter_better($active_plugins, fn ($p) => $p !== $this_plugin_from_active_plugins)));
...
\NeoReplace\NeoGlobal\add_action_hook("neo_init", function () {
    ...
    foreach (\NeoReplace\NeoEntrypoint\get_neo_active_plugins() as $active_plugin) {
        ...
        foreach (\NeoReplace\NeoGlobal\iterate_all_files($plugin_path) as $filepath) {
            if (!str_ends_with($filepath, ".js")) { continue; }
            if (str_contains($filepath, "thirdparty")) { continue; }
            $rel_path = \NeoReplace\NeoGlobal\make_plugin_file_path_relative($filepath);
    ...
    \NeoReplace\NeoGlobal\enqueue_js_variable_backend_and_frontend("neoInterfaceModuleUrls", $interface_module_urls);
});
...
\NeoReplace\NeoGlobal\add_action_hook("neo_init", function () {
    ...
    \NeoReplace\NeoGlobal\enqueue_js_variable_backend_and_frontend("neoAvailableModules", $available_modules);
});
